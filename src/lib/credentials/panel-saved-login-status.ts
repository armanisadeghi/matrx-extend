...
export function parsePanelSavedLoginStatus(value: unknown): PanelSavedLoginSnapshot {
  ...
  if (value.status === 'none' || value.status === 'disabled') {
    return exactKeys(value, ['status', 'itemIds']) &&
      Array.isArray(value.itemIds) &&
      value.itemIds.length === 0
      ? { status: value.status, itemIds: [] }
      : unavailable;
  }
  if (
    value.status !== 'ready' ||
    !exactKeys(value, ['status', 'offerId', 'itemIds', 'matches', 'pageUrl', 'frameId'])
  )
    return unavailable;
  if (
    typeof value.offerId !== 'string' ||
    !/^[0-9a-f]{36}$/.test(value.offerId) ||
  ...
  try {
    ...
    if (
      !isSafeDestination(url) ||
      url.username ||
      url.password ||
      value.pageUrl !== `${url.origin}${url.pathname}`
    )
      return unavailable;
  } catch {
    return unavailable;
  ...
  for (const item of value.matches) {
    if (
      !record(item) ||
      !exactKeys(item, ['item_id', 'display_name']) ||
      typeof item.item_id !== 'string' ||
      !ids.has(item.item_id) ||
      matched.has(item.item_id) ||
