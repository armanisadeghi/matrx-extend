...
export function mountGenerationTargetRegistry(): GenerationTargetRegistry {
  if (window.__matrx_generation_target_registry__)
    return window.__matrx_generation_target_registry__;
  ...
  const entries = new Map<
    string,
    {
      node: WeakRef<HTMLInputElement>;
      group: WeakRef<Element>;
      hosts: Array<{ host: WeakRef<Element>; parent: WeakRef<Node>; root: WeakRef<Node> }>;
      documentId: string;
      expiresAt: number;
      claimed: boolean;
      members: string[] | null;
  ...
  const hostsFor = (
    input: HTMLInputElement,
  ): Array<{ host: WeakRef<Element>; parent: WeakRef<Node>; root: WeakRef<Node> }> => {
  ...
  const sameHosts = (
    input: HTMLInputElement,
    expected: Array<{ host: WeakRef<Element>; parent: WeakRef<Node>; root: WeakRef<Node> }>,
  ) => {
    const hosts = hostsFor(input);
    return (
      hosts.length === expected.length &&
      hosts.every((placement, index) => {
        const saved = expected[index];
        return (
          !!saved &&
          saved.host.deref() === placement.host.deref() &&
          saved.parent.deref() === placement.parent.deref() &&
          saved.root.deref() === placement.root.deref()
        );
      })
    );
  };
  const purge = () => {
    ...
    for (const [id, entry] of entries) {
      if (
        entry.expiresAt <= now ||
        entry.node.deref() === undefined ||
        entry.group.deref() === undefined
      )
        entries.delete(id);
    }
    for (const [id, entry] of inputs) {
      ...
      if (
        ...
        input.getRootNode() !== entry.root.deref() ||
        !sameHosts(input, entry.hosts)
      )
        inputs.delete(id);
    }
  };
  const registry: GenerationTargetRegistry = {
    register(input, group, documentId, expiresAt) {
      ...
      if (
        input.ownerDocument !== documentRef ||
        group.ownerDocument !== documentRef ||
        !group.isConnected ||
        !input.isConnected ||
        !documentId ||
        !Number.isFinite(expiresAt) ||
        expiresAt <= Date.now() ||
        expiresAt - Date.now() > GENERATED_SECRET_TTL_MS
      )
        return null;
      const id = opaqueId();
      if (!id) return null;
      const hosts = hostsFor(input);
      entries.set(id, {
        node: new WeakRef(input),
        group: new WeakRef(group),
        hosts,
        documentId,
        expiresAt,
        claimed: false,
    ...
    registerInput(input) {
      ...
      inputs.set(id, {
        node: new WeakRef(input),
        hosts: hostsFor(input),
        root: new WeakRef(input.getRootNode()),
        documentId: null,
        expiresAt,
      });
      window.setTimeout(() => {
        const entry = inputs.get(id);
        if (entry?.expiresAt === expiresAt && entry.expiresAt <= Date.now()) inputs.delete(id);
      }, REGISTERED_INPUT_TTL_MS);
      return id;
    },
    resolveInput(id, documentId) {
      ...
      if (
        ...
        input.getRootNode() !== entry.root.deref() ||
        !sameHosts(input, entry.hosts) ||
        (entry.documentId !== null && entry.documentId !== documentId)
      )
        return null;
      ...
      if (
        selected.some(
          (entry) =>
            ...
            entry.expiresAt !== expiresAt ||
            !entry.members ||
            entry.members.length !== ids.length ||
            entry.members.some((id, index) => id !== ids[index]),
        )
      )
        return false;
