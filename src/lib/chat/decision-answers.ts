 ...
 * The extension has no decision card; its stream logged the typed `data`
 * event and its history reader skipped the part, so a decision reply arrived as
 * an EMPTY bubble — a paid, finished
...
/**
 * The verdict as a plain markdown render block, so it lands in the message at
 * its arrival position through the same block path every server block takes.
 */
export function decisionRenderBlock(
  payload: unknown,
  blockIndex: number,
): InboundRenderBlock | null {
  const text = decisionAnswersText(payload);
  if (!text) return null;
  return {
    blockId: `decision_answers_${blockIndex}`,
    blockIndex,
    type: 'markdown',
    status: 'complete',
    content: text,
