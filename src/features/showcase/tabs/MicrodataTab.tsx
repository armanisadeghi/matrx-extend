      ...
      <div className="space-y-3 px-3 pb-3">
        <div className="space-y-1">
          ...
          <div className="text-xs text-muted-foreground">
            Schema.org microdata: <code className="rounded bg-secondary/60 px-1">[itemscope]</code>{' '}
            blocks. Best on Yelp, Amazon, recipes, and local-business sites.
          </div>
        </div>
        ...
        <div className="flex gap-2">
          <Input
            ...
            placeholder="Filter by type (e.g. Restaurant)"
            className="h-9 rounded-full border-0 bg-secondary/40 text-xs focus-visible:ring-1"
          />
          <Button
            onClick={() => void run(filter ? { itemtype: filter } : {})}
        ...
        {rows && <ResultPreview rows={rows} emptyHint="No matching microdata items." />}
