...
- Privacy → "Offer saved logins on sign-in forms" (`offerSavedLoginsEnabled`, default ON) gates metadata-only focused-field matching. On focus, the extension uses only page origin/path and a generated field selector; it never reads the field value. A user click is required before materialization/fill; it never submits a form. This disclosure belongs in the Chrome Web Store privacy submission for the feature release.
