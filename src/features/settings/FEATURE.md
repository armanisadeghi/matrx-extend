...
- Privacy → "Offer to save logins to the Vault" (`captureLoginsEnabled`, default ON, signed-in only) gates the page-driven save prompt; "Never ask on these sites" lists the per-origin opt-outs (`src/lib/credentials/capture-settings.ts`) with **Ask again**. See `docs/SYSTEM_STATE.md` § Save this login?.
