...
- **Steps:** 1) As member A in workspace A, capture an article page and press Save. 2) Press "Open this Source (opens in the web app)". 3) Open Saved captures; check the capturing-member label, Article, Details, Data and Copy; edit the text and Save changes. 4) As member B of workspace A, open Saved captures, inspect the same Source and its captured-by label, then inspect the delete confirmation without confirming it. 5) Switch to workspace B, where that URL has not been saved, and revisit the page. With a fresh extension profile for a signed-in member of both workspaces, revisit it before choosing a workspace. 6) Return to workspace A; with the network off (DevTools → Offline on the side panel), capture and Save another page.
