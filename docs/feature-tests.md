...
- **Cross-context and role case:** complete sign-in from another extension context, then reopen the side panel; the shown account and admin access follow the verified stored session, even if an older sign-in notice arrives afterward. If the admin-role read is temporarily unavailable, the account remains signed in, admin tabs stay hidden, the banner says the role check failed, and **Try again** repeats that check without opening OAuth.
...
- **Non-disruptive acceptance:** Set `MATRX_VAULT_CANARY_DISPLAY=HEADLESS_NO_CLIPBOARD`, generator transport and read-only admission modes, and an explicit full Chrome-for-Testing executable. This runs genuine panel controls and targeted browser keyboard events without OS activation or clipboard access. Copy stays explicitly untested. Missing/unknown display modes refuse; `HEADED` additionally requires `MATRX_VAULT_CANARY_FOREGROUND=ALLOW_FOREGROUND_TEST`. Check these launch gates with `node tests/browser/vault-display-mode-admission.test.cjs`. A resolved initial missing-workspace notice is acknowledged only through its real Dismiss button after admin identity and the selected organization are verified; other notices are not dismissed.

### Inline saved-login suggestions
- **What it does:** When on-page assistance is explicitly enabled, shows a Matrx control on an eligible top-frame sign-in field; choosing an account fills available login fields without submitting. The default quiet setting keeps assistance in the toolbar and side panel.
- **Where to test:** An admin-owned disposable login page with the installed extension.
- **Steps:** Enable on-page saved-login assistance, focus a password field, click the Matrx control, choose a saved account, and inspect the form before submitting yourself. Switch back to the quiet setting and verify the page control disappears.
- **Expected:** Account names only are shown. The chosen username/password fields fill, no navigation or submit occurs, Escape returns focus, and changing page, sign-in, organization, or the Privacy toggle makes the old choice unavailable. If page code changes or replaces an attempted field before rollback, the panel says to review the login fields and never submits.
- **Edge cases worth poking:** username-only continuation, multiple accounts and keyboard arrows, OTP/new-password/confirmation/readonly/GET/cross-origin forms, and a reload while the chooser is open. On a short viewport, Tab to the Matrx title, confirm its focus ring is visible, and verify the account list scrolls within the remaining viewport. While a fill is waiting on page validation, sign out, switch organization, or turn off the Privacy toggle: the materialized choice must refuse; unchanged attempted fields return to their prior values, while page-owned edits and replacement controls remain untouched for review.
...
- **What it does:** Save sends the capture through the landing door (`POST /sources/land`) as a kept, personal Source: the article cut into H1–H3 sections, the full capture kept as the original (S3), the collectors on the Source's structured data. If the server refuses or cannot be reached, the capture stays on this device under an amber **Unsaved — retry** card (with the server's sentence) until a retry lands; the unsaved-edits guard stays armed. The Saved captures tab lists Sources saved from the extension (read directly under RLS), opens the original for Details / Data / Copy, saves edits beside the original (`/sources/{id}/edit`), and deletes by soft delete. Recognition ("Saved <date>") and the agent's `prior_capture` look the page up by the door's canonical URL.
- **Where to test:** Side panel → Scrape, then Saved captures.
- **Steps:** 1) Capture any article page and press Save. 2) Press "Open this Source (opens in the web app)". 3) Open Saved captures; open the new card; check Article, Details, Data and the Copy menu; Edit the text and Save changes; Delete it. 4) With the network off (DevTools → Offline on the side panel), capture and Save again.
- **Expected:** 1) The button reads Saved and a link to the Source appears; it opens the web app's document viewer (`/knowledge/viewer/<id>`) until the Source screen ships. If you edited the article before saving, the saved Source carries your edited text; the original page data is kept unedited. 3) The card is there within seconds; Details shows image/link counts from the original; after an edit the Article pane says it shows your edited version and the original is kept. 4) The amber "Unsaved — retry" card appears with the page title and a sentence; Re-capture still asks before discarding edits; closing and reopening the side panel still shows the card; turning the network back on and pressing Retry save removes the card and the page appears in Saved captures.
...
- Real capture decisions require a genuine extension side panel. A `sidepanel.html` browser tab is a different sender and cannot certify the capture journey. The accepted harness uses trusted browser input through the actual side-panel target; normal-tab substitution is refused.
...
- September 19 evidence: genuine-panel run `d02225ac-a08c-40fd-9674-668fce930b3c` independently passed exact-target Update among four accounts and Save as new without website submission. All five owned fixtures completed GET 200 → DELETE 204 → GET 404; baseline, logout and profile cleanup passed. Committed runner `0e5a2e6` matches the proof hash. Enrolled-MFA semantics, lost-response recovery, restart and distributed-artifact acceptance remain open. Durable sanitized evidence lives in common-docs/projects/credential-sharing-browser-login/evidence/task1-save-update-2026-09-19.json.
