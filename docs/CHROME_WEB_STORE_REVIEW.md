...
public item `hnfolienncfklkgmdjjmhhegglimlamg`. Versions `0.1.74`–`0.2.1` are
superseded and must not be uploaded. Version `0.2.2` is the current validated
Store candidate at `.output/matrx-extend-0.2.2-store.zip` (SHA-256
`8a57ac8c513d3cbbd57de6139c2bceec992ff2649397dd1be65b6ccae1edd1de`). It has
...
| 2026-08-20 18:40 PDT | Version `0.2.2` superseded the stale `0.2.1` candidate by including all four source fixes committed after tag `v0.2.1`: terminal client-tool lifecycle resolution, truthful conversation-history hydration, the browser Mandate default, and stabilized extension defaults and auxiliary flows. An isolated exact-`origin/main` release passed TypeScript, all **460** tests, strict schema routing, 82/82 tool-registry drift verification, migration ledger (15 applied / 0 pending / 0 drifted), Store package validation, the Chrome Web Store approval-risk gate, and separate local packaging. The Store manifest is version `0.2.2`, has no development key or toolbar popup, and retains the canonical side panel. Store artifact `.output/matrx-extend-0.2.2-store.zip` SHA-256 `8a57ac8c513d3cbbd57de6139c2bceec992ff2649397dd1be65b6ccae1edd1de`; local artifact SHA-256 `b4d899f31c63044a182542de02ff20b32174176353e2f3f09080cbe91ee9fba1`. It has not been uploaded. East remains on the public `0.1.72`-compatible tool contract until a compatible Store candidate is actually published. |
