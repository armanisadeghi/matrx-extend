...
- Active builder `/root/seo_guest_detail_build` repairs e3eab0f1 guest-only T09 after peerfe20b274: compare all schema chips, improve independent readability/schema oracle or narrow claims. No runtime yet; fresh sourcepeer required before root guarded seo-guest-008. Ruling: independently read raw DOM counts may share straightforward business rules without being circular; copying complex product algorithms is not independent verification. Cost if wrong is a false-pass; preserve bounded claims and actual-page positive controls. PrimaryChrome remains .72; owned .73 runs are separate.
