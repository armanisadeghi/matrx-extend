...
- [ENH] Receipt coverage: WebMCP path now emits signed receipts; receipt schema v2 adds optional origin tag (agent/pilot/parallel/webmcp) — backward compatible with v1. `handleWebmcpCall` in [src/lib/tools/dispatch.ts](../src/lib/tools/dispatch.ts) now emits its own partial + completed receipts (origin='webmcp') since that path bypasses the streaming dispatcher's chunk listener; the streaming path classifies origin via `detectOrigin(ctx)` (runId starts with `parrun-` → 'parallel'; conversation matches active Pilot session → 'pilot'; otherwise 'agent'). Receipt schema bumped to `v: 2` ([src/lib/audit/receipt.ts](../src/lib/audit/receipt.ts)) with `SUPPORTED_SCHEMA_VERSIONS = [1, 2]` so old receipts still verify — absent `origin` on v1 stays absent through canonical-JSON re-serialization, signature still matches. Settings → Advanced → Audit key gains a "Recent receipts" panel with chip-set origin filter (counts per origin) ([src/features/settings/AuditKeyCard.tsx](../src/features/settings/AuditKeyCard.tsx)). Unit tests in [tests/unit/receipt.test.ts](../tests/unit/receipt.test.ts) cover all four origin tags + a v1 backward-compat case + tampering detection (9 tests, all passing). Test setup ([tests/setup.ts](../tests/setup.ts)) now installs a shared chrome.storage.local mock for the suite. Typecheck + wxt build clean. 2026-05-07 (PLACEHOLDER_SHA)
