...
- **Capability-based discovery (2026-05-01)** — every chat ships a single
  ...
  via [`types/server-handoff/browser-dom-capability.json`](./types/server-handoff/browser-dom-capability.json)
  — single source of truth, regenerated by `pnpm catalog:tools`.
...
**Handoff artifacts the server consumes:**

- [`types/server-handoff/browser-dom-capability.json`](./types/server-handoff/browser-dom-capability.json) —
  the routing rule. Drop next to the `load_browser_tools` handler. Contains:
  - `payload_schema` (JSON Schema for `state["browser-dom"]`)
  - `always_on_tools` (just `load_browser_tools`)
  - `category_routing` (`{ category → tool_names[] }`)
  - `tool_metadata` (`{ name → { tier, category, admin_only, required_optional_permissions } }`)
  - `totals` (sanity counts)
- [`.research/server-handoff-browser-dom-capability.md`](./.research/server-handoff-browser-dom-capability.md) —
