...
const FRESH_PROOF_PATH = path.join(
  REPO_ROOT,
  '.matrx/task1-active/firefox-sidebar-probe/authenticated-harness/auth-runs',
  FRESH_RUN_ID,
  'proof.json',
);
...
function assertExactPaths(failedProofPath, freshProofPath) {
  assert.equal(
    path.resolve(failedProofPath),
    FAILED_PROOF_PATH,
    'historical_reconciliation_failed_proof_path_mismatch',
  );
  assert.equal(
    path.resolve(freshProofPath),
    FRESH_PROOF_PATH,
    'historical_reconciliation_fresh_proof_path_mismatch',
  );
}
...
function assertFailedRecord(record) {
  ...
  assert.equal(
    ...
    'historical_reconciliation_failed_mode_mismatch',
  );
  ...
  assert.equal(
    ...
    'historical_reconciliation_failed_verdict_mismatch',
  );
  assert.equal(
    exactSet(record?.ownedFixtureIds, HISTORICAL_FIXTURE_IDS),
    true,
    'historical_reconciliation_failed_fixture_ids_mismatch',
  );
  const cleanup = record?.cleanup;
  assert.equal(
    cleanup?.receiptReconciled,
    true,
    'historical_reconciliation_failed_receipt_cleanup_missing',
  );
  assert.equal(
    cleanup?.createdItemsGone,
    true,
    'historical_reconciliation_failed_fixture_cleanup_missing',
  );
  assert.equal(
    cleanup?.localAuthLogoutStatus,
    204,
    'historical_reconciliation_failed_local_auth_cleanup_missing',
  );
  assert.equal(
    cleanup?.remoteAuthRevocationStatus,
    204,
    'historical_reconciliation_failed_remote_auth_cleanup_missing',
  );
  assert.equal(
    cleanup?.browserClosed,
    true,
    'historical_reconciliation_failed_browser_cleanup_missing',
  );
  assert.equal(
    cleanup?.localFixtureServerClosed,
    true,
    'historical_reconciliation_failed_fixture_server_cleanup_missing',
  );
  assert.equal(
    cleanup?.profileRemoved,
    true,
    'historical_reconciliation_failed_profile_cleanup_missing',
  );
}
...
function assertFreshRecord(record) {
  ...
  assert.equal(
    ...
    'historical_reconciliation_fresh_mode_mismatch',
  );
  assert.equal(record?.ok, true, 'historical_reconciliation_fresh_run_not_complete');
  assert.equal(
    record?.vaultMutationRequests,
    0,
    'historical_reconciliation_fresh_mutation_observed',
  );
  assert.equal(
    record?.baselineReconciled,
    true,
    'historical_reconciliation_fresh_baseline_not_reconciled',
  );
  assert.equal(
    record?.baseline?.itemCount,
    34,
    'historical_reconciliation_clean_baseline_count_changed',
  );
  assert.equal(
    record?.baseline?.metadataSha256,
    CLEAN_BASELINE_SHA256,
    'historical_reconciliation_clean_baseline_metadata_changed',
  );
  const reconciliation = record?.historicalReconciliation;
  assert.equal(
    reconciliation?.failedRunId,
    FAILED_RUN_ID,
    'historical_reconciliation_linked_failed_run_mismatch',
  );
  assert.equal(
    reconciliation?.failedProofSha256,
    FAILED_PROOF_SHA256,
    'historical_reconciliation_linked_failed_hash_mismatch',
  );
  assert.equal(
    reconciliation?.cleanBaselineBefore,
    true,
    'historical_reconciliation_baseline_before_missing',
  );
  assert.equal(
    reconciliation?.cleanBaselineAfter,
    true,
    'historical_reconciliation_baseline_after_missing',
  );
  const statuses = reconciliation?.ownedFixtureStatuses;
  assert.equal(
    Array.isArray(statuses) &&
      statuses.length === HISTORICAL_FIXTURE_IDS.length &&
      new Set(statuses.map((entry) => entry?.id)).size === HISTORICAL_FIXTURE_IDS.length &&
      statuses.every(
        (entry) => HISTORICAL_FIXTURE_IDS.includes(entry?.id) && entry?.status === 404,
      ),
    true,
  ...
  assert.equal(
    record?.nativeSignOutNetwork?.addonLogout204Observed,
    true,
    'historical_reconciliation_fresh_native_logout_204_missing',
  );
  assert.equal(
    record?.nativeSignOutNetwork?.addonLogoutRequestObserved,
    true,
    'historical_reconciliation_fresh_native_logout_request_missing',
  );
  assert.equal(
    record?.nativeSignOutNetwork?.nodeFallbackAttempted,
    false,
    'historical_reconciliation_fresh_native_logout_used_fallback',
  );
  assert.equal(
    record?.nativeVaultNetwork?.addonItemsRequestObserved,
    true,
    'historical_reconciliation_fresh_native_vault_request_missing',
  );
  assert.equal(
    record?.nativeVaultNetwork?.addonItems2xxObserved,
    true,
    'historical_reconciliation_fresh_native_vault_2xx_missing',
  );
  assert.equal(
    Number.isInteger(record?.nativeVaultNetwork?.responseStatus) &&
      record.nativeVaultNetwork.responseStatus >= 200 &&
      record.nativeVaultNetwork.responseStatus < 300,
    true,
    'historical_reconciliation_fresh_native_vault_response_not_2xx',
  );
  assert.equal(
    record?.networkObserver?.dropped,
    0,
    'historical_reconciliation_fresh_network_events_dropped',
  );
  assert.equal(
    record?.networkObserver?.observerErrors,
    0,
    'historical_reconciliation_fresh_network_observer_errors',
  );
  ...
  assert.equal(
    cleanupNetwork?.dropped,
    0,
    'historical_reconciliation_fresh_cleanup_network_events_dropped',
  );
  assert.equal(
    cleanupNetwork?.observerErrors,
    0,
    'historical_reconciliation_fresh_cleanup_network_observer_errors',
  );
  assert.equal(
    record?.nativeOsInputUsed,
    false,
    'historical_reconciliation_fresh_native_os_input_used',
  );
  assert.equal(
    record?.storedSessionSeeded,
    false,
    'historical_reconciliation_fresh_session_seeded',
  );
  assert.deepEqual(
    record?.cleanupErrors,
    [],
    'historical_reconciliation_fresh_cleanup_errors_present',
  );
  assert.equal(
    record?.terminalPhase,
    'complete',
    'historical_reconciliation_fresh_terminal_phase_incomplete',
  );
}
 ...
 */
function assertHistoricalChromeReconciliation({
  failedProofPath,
  failedRaw,
  freshProofPath,
  freshRaw,
  ...
  assert.equal(
    sha256(failedRaw),
    FAILED_PROOF_SHA256,
    'historical_reconciliation_failed_proof_hash_changed',
  );
  assert.equal(
    sha256(freshRaw),
    FRESH_PROOF_SHA256,
    'historical_reconciliation_fresh_proof_hash_changed',
  );
  let failed;
  let fresh;
  try {
    ...
    throw new Error('historical_reconciliation_failed_proof_invalid_json');
  }
  try {
    fresh = JSON.parse(freshRaw);
  } catch {
    throw new Error('historical_reconciliation_fresh_proof_invalid_json');
  }
