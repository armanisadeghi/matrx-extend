...
(async () => {
  assert(
    process.env.MATRX_HEADLESS_CLIPBOARD_PROBE === 'RUN_REVIEWED_ISOLATION_PROBE',
    'probe_not_enabled',
  );
  const root = path.resolve(
    __dirname,
    '../../.matrx/task1-active/clipboard-isolation',
    crypto.randomUUID(),
  );
  await fs.mkdir(root, { recursive: true, mode: 0o700 });
  const proof = {
    schema: 1,
    scope: 'native clipboard isolation between two owned headless Chrome processes',
    expectedVersion,
    credentialsRead: false,
    authenticationAttempted: false,
    hostClipboardApiCalled: false,
    platformIsolationBasis:
      'exact-runtime Chromium headless source uses ClipboardNonBacked; separately checked by process isolation',
    checks: {},
    cleanup: {},
    ok: false,
  };
  ...
  try {
    server = http.createServer((req, res) => {
      ...
      res.end('<!doctype html><title>Owned clipboard isolation probe</title><input id="focus">');
    });
    ...
    const pages = [];
    for (let i = 0; i < 2; i++) {
      const profile = path.join(root, `profile-${i}`);
      profiles.push(profile);
      await fs.mkdir(profile, { mode: 0o700 });
      const context = await chromium.launchPersistentContext(profile, {
        headless: true,
        executablePath,
        args: ['--enable-automation', '--no-first-run', '--no-default-browser-check'],
      });
      ...
      assert(
        commandLine.arguments.some((arg) => arg === '--headless' || arg === '--headless=new'),
        'headless_flag_missing',
      );
      assert(
      ...
      for (const entry of processes.processInfo) {
        assert(Number.isSafeInteger(entry.id) && entry.id > 0, 'process_id_invalid');
    ...
    const initial = await Promise.all(
      pages.map((page) => page.evaluate(async () => (await navigator.clipboard.readText()) === '')),
    );
    assert(initial.every(Boolean), 'fresh_clipboard_not_empty');
    proof.checks.freshClipboardsEmpty = true;
    const canaryA = crypto.randomUUID(),
      canaryB = crypto.randomUUID();
    ...
    assert(
      await pages[1].evaluate(async () => (await navigator.clipboard.readText()) === ''),
      'clipboard_a_leaked_to_b',
    );
    ...
    proof.cleanup.serverClosed = server?.listening === false;
    await Promise.all(profiles.map((profile) => fs.rm(profile, { recursive: true, force: true })));
    proof.cleanup.profilesRemoved = (
      await Promise.all(
        profiles.map((profile) =>
          fs.lstat(profile).then(
            () => false,
    ...
    proof.ok =
      !failure &&
      Object.values(proof.checks).every((value) => value === true) &&
      Object.values(proof.cleanup).every((value) => value === true);
    await fs.writeFile(path.join(root, 'proof.json'), JSON.stringify(proof, null, 2) + '\n', {
      mode: 0o600,
    });
    process.stdout.write(
      (proof.ok ? 'PASS' : 'FAIL') + ': ' + path.join(root, 'proof.json') + '\n',
    );
