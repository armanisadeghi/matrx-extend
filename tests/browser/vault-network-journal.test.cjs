...
class Flat {
  constructor() { this.listeners = new Map(); this.calls = []; this.fatal = false; this.plans = []; this.heldDetaches = []; this.holdDetach = false; this.networkGates = new Map(); this.networkReleases = new Map(); this.failDetach = new Set(); }
  on(method, listener) { this.listeners.set(method, listener); }
  off(method, listener) { if (this.listeners.get(method) === listener) this.listeners.delete(method); }
  emit(method, params, sessionId) { this.listeners.get(method)?.(params, sessionId); }
  plan(plan) { this.plans.push(plan); }
  gateNetwork(sessionId) { this.networkGates.set(sessionId, true); }
  releaseNetwork(sessionId) { this.networkReleases.get(sessionId)?.(); }
  emitAttach(plan) { this.emit('Target.attachedToTarget', { sessionId: plan.sessionId, targetInfo: { targetId: plan.actualTargetId ?? plan.targetId, type: plan.type ?? 'page' }, waitingForDebugger: true }); }
  async send(method, params = {}, sessionId) {
    this.calls.push({ method, params, sessionId });
    if (method === 'Target.setAutoAttach' && params.autoAttach === false && Object.hasOwn(params, 'filter')) throw new Error('no_disable_filter');
    if (method === 'Network.enable' && this.networkGates.has(sessionId)) {
      return new Promise((resolve) => this.networkReleases.set(sessionId, () => { this.networkGates.delete(sessionId); resolve({}); }));
    }
    if (method === 'Target.attachToTarget') {
      const plan = this.plans.shift(); if (!plan) throw new Error('missing_plan');
    ...
    if (method === 'Target.detachFromTarget') {
      ...
      else queueMicrotask(() => this.emit('Target.detachedFromTarget', { sessionId: params.sessionId }));
    }
    return {};
  }
  flushDetaches() { for (const sessionId of this.heldDetaches.splice(0)) this.emit('Target.detachedFromTarget', { sessionId }); }
  async detach() { this.closed = true; }
}
const newJournal = (cdp) => createVaultNetworkJournal({ cdp, apiOrigin: API, ownedLocalOrigin: 'http://127.0.0.1:40123' });
const request = (cdp, session, id, url, method = 'GET') => cdp.emit('Network.requestWillBeSent', { requestId: id, request: { url, method } }, session);
const response = (cdp, session, id, status) => cdp.emit('Network.responseReceived', { requestId: id, response: { status } }, session);

(async () => {
  const cdp = new Flat(), journal = newJournal(cdp);
  await journal.start();
  ...
  assert.deepEqual(cdp.calls.slice(1, 4).map((call) => [call.method, call.sessionId]), [['Target.attachToTarget', undefined], ['Network.enable', 'before'], ['Runtime.runIfWaitingForDebugger', 'before']]);
  request(cdp, 'before', 'old', API + '/api/vault/items'); response(cdp, 'before', 'old', 204);
  ...
  request(cdp, 'after', 'new', API + '/api/vault/items'); response(cdp, 'after', 'new', 200);
  await journal.assertCoverage();
  ...
  request(cdp, 'after', 'fresh', API + '/api/vault/items'); response(cdp, 'after', 'fresh', 200);
  await journal.assertCoverage();

  cdp.plan({ sessionId: 'manual', targetId: 'raced', before: true, raceAuto: { sessionId: 'automatic', targetId: 'raced' } });
  const raced = await journal.bindPanelTarget('raced');
  assert.equal(raced.sessionId, 'manual');
  assert.ok(cdp.calls.some((call) => call.method === 'Target.detachFromTarget' && call.params.sessionId === 'automatic'));
  await journal.settle();
  assert.equal(journal.snapshot().boundTargetSessionCount, 1);

  cdp.gateNetwork('inverse-manual');
  cdp.plan({ sessionId: 'inverse-manual', targetId: 'inverse', before: true, raceAuto: { sessionId: 'inverse-auto', targetId: 'inverse' } });
  ...
  assert.ok(cdp.calls.some((call) => call.method === 'Target.detachFromTarget' && call.params.sessionId === 'inverse-manual'));
  ...
  await new Promise((resolve) => setImmediate(resolve));
  assert.ok(cdp.calls.some((call) => call.method === 'Target.detachFromTarget' && call.params.sessionId === 'late-auto'));
  ...
  const mismatchCdp = new Flat(), mismatch = newJournal(mismatchCdp);
  ...
  const missingCdp = new Flat(), missing = newJournal(missingCdp);
  await missing.start();
  ...
  const duplicateCdp = new Flat(), duplicate = newJournal(duplicateCdp);
  ...
  const cleanupCdp = new Flat(), cleanup = newJournal(cleanupCdp);
  await cleanup.start();
  cleanupCdp.plan({ sessionId: 'manual-cleanup', targetId: 'close-me' });
  await cleanup.bindPanelTarget('close-me');
  await cleanup.dispose();
  assert.ok(cleanupCdp.calls.some((call) => call.method === 'Target.detachFromTarget' && call.params.sessionId === 'manual-cleanup'));
  assert.equal(cleanupCdp.closed, true);

  const failedDetachCdp = new Flat(), failedDetach = newJournal(failedDetachCdp);
  ...
  assert.deepEqual(cdp.calls.findLast((call) => call.method === 'Target.setAutoAttach').params, { autoAttach: false, waitForDebuggerOnStart: false, flatten: true });
  process.stdout.write('PASS: explicit panel binds correlate sessions, reset epochs, and clean manual observers\n');
