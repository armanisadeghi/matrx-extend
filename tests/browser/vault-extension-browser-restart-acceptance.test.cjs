...
const scenario = (overrides = {}) => {
  const state = { replacementClosed: false, replacementJournalDisposed: false, writes: 0, order: [] };
  ...
  const context = { close: async () => { state.replacementClosed = true; } };
  return {
    state,
    args: {
      ...
      extensionId,
      workerUrl,
      initialContext: { close: async () => { state.initialClosed = true; state.order.push('initial-context-close'); } },
      initialPanel: { dispose: async () => { state.initialPanelDisposed = true; state.order.push('initial-panel-dispose'); }, targetId: 'old-panel' },
      initialWorker: worker(),
      initialWorkerTargetId: 'old-worker',
      initialPanelTargetId: 'old-panel',
      initialBrowserPid: 111,
      initialCdp: { detach: async () => { state.initialCdpDetached = true; state.order.push('initial-cdp-detach'); } },
      initialJournal: { dispose: async () => { state.initialJournalDisposed = true; state.order.push('initial-journal-dispose'); } },
      ...
      assertLaunchProvenance: async ({ profile, launchOptions, browserPid }) =>
        profile === '/owned/disposable-profile' && launchOptions.executablePath === '/owned/chrome' && browserPid === 222,
      ...
      verifyOwnedBrowserProcess: async ({ profile, launchOptions, browserPid }) =>
        profile === '/owned/disposable-profile' && launchOptions.args[0] === '--headless=new' && browserPid === 222 && overrides.newOwned !== false,
      connectOwnedCdp: async () => {
        state.order.push('replacement-cdp-connect');
        return {
          send: async () => ({ targetInfos: targets }),
          detach: async () => { state.replacementCdpDetached = true; },
        };
      },
      createReplacementJournal: async () => ({
        start: async () => { state.order.push('replacement-journal-start'); },
        bindPanelTarget: async () => { state.order.push('replacement-journal-bind'); },
        settle: async () => { state.order.push('replacement-journal-settle'); },
        ...
        dispose: async () => { state.replacementJournalDisposed = true; },
      }),
      acquireWorker: async () => { state.order.push('replacement-worker-acquire'); return worker(); },
      openPanel: async () => { state.order.push('replacement-panel-open'); return { targetId: overrides.panelTargetId ?? 'new-panel', dispose: async () => {} }; },
...
(async () => {
  ...
  assert.ok(
    passed.state.order.indexOf('initial-journal-dispose') < passed.state.order.indexOf('initial-cdp-detach'),
    'initial journal must dispose before its CDP transport detaches',
  );
  assert.ok(
    passed.state.order.indexOf('replacement-journal-start') < passed.state.order.indexOf('replacement-worker-acquire'),
    'replacement journal must start before replacement worker use',
  );
  assert.ok(
    passed.state.order.indexOf('replacement-cdp-connect') < passed.state.order.indexOf('replacement-journal-start'),
  ...
  assert.ok(
    passed.state.order.indexOf('replacement-journal-settle') < passed.state.order.indexOf('replacement-journal-coverage'),
    'replacement journal must settle before coverage is asserted',
  );

  for (const [name, overrides, code, launched, journalCreated] of [
    ['old process alive', { oldExited: false }, 'browser_restart_previous_process_alive', false, false],
    ['reused process pid', { browserPid: 111 }, 'browser_restart_process_pid_reused', true, false],
    ['unverified launch provenance', { assertLaunchProvenance: async () => false }, 'browser_restart_launch_provenance_unverified', true, false],
    ['unowned replacement process', { newOwned: false }, 'browser_restart_replacement_process_unowned', true, false],
    ['forged worker target', { targets: [{ targetId: 'new-panel', type: 'page', url: panelUrl }] }, 'browser_restart_replacement_worker_ambiguous', true, true],
    ['forged panel target', { panelTargetId: 'forged-panel' }, 'browser_restart_panel_target_unverified', true, true],
    ['vault write during restart', { vaultWriteCount: (() => { let count = 0; return () => count++; })() }, 'browser_restart_evidence_incomplete', true, true],
    ['replacement journal mutation', { replacementMutations: 1 }, 'browser_restart_evidence_incomplete', true, true],
    ['replacement journal coverage failure', { coverageFailure: true }, 'vault_network_observer_failed', true, true],
  ]) {
    ...
    assert.equal(failed.state.replacementJournalDisposed, journalCreated, `${name} journal cleanup mismatch`);
  }
  process.stdout.write('PASS: owned restart rejects forged process, launch, worker, and panel evidence\n');
})().catch((error) => {
  console.error(error);
  process.exitCode = 1;
});
