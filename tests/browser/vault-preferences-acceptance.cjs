...
exports.runVaultPreferencesChecks = async ({
  ...
  try {
    ...
    checkpoint('preferences_create_owned_page');
    page = await context.newPage();
    checkpoint('preferences_navigate_owned_page');
    await page.goto(url.href, { waitUntil: 'domcontentloaded' });
    checkpoint('preferences_resolve_owned_tab');
    tabId = await tabFor(worker, url.href, wait);
    checkpoint('preferences_wait_for_bridge');
    await waitForBridge(worker, tabId, wait);
    checkpoint('preferences_activate_owned_tab');
    await page.bringToFront();
    await focusOwnedBrowser(tabId);
    checkpoint('preferences_open_settings');
    await ensurePrivacyOpen();
    checkpoint('preferences_read_initial_switches');
