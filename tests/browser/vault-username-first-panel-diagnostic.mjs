...
const bundle = await build({
  stdin: {
    contents: wrapper,
    resolveDir: root,
    sourcefile: 'username-first-panel-entry.ts',
    loader: 'ts',
  },
  bundle: true,
  format: 'iife',
...
try {
  ...
  await page.evaluate(() => {
    ...
    globalThis.chrome = {
      ...
      tabs: {
        get: async () => ({ id: 7, windowId: 1, active: true }),
      ...
      webNavigation: {
        getFrame: async () => ({ documentId: 'doc-1', url: location.href, parentFrameId: -1 }),
      },
  ...
  result = await page.evaluate(async () => {
    ...
    await globalThis.__panelInvoke(
      {
        __matrx: true,
        kind: 'credential-suggestions:focus-owner',
        payload: { stamp: 1, sequence: 1 },
      },
      sender,
    );
    ...
    const query = await globalThis.__panelInvoke(
      {
        __matrx: true,
        kind: 'credential-suggestions:query',
        payload: { field: { kind: 'registered_input', id } },
      },
      sender,
    );
    const fill = await globalThis.__panelInvoke(
      {
        __matrx: true,
      ...
      { id: 'fixture-extension', url: 'chrome-extension://fixture-extension/sidepanel.html' },
    );
    return {
      query: query?.status ?? null,
      fill: fill?.status ?? null,
      value: input.value,
      fieldKeys: globalThis.__panelDiag.materialize?.fieldKeys ?? null,
...
process.stdout.write(
  `${JSON.stringify({ ok: true, scope: 'production-host-dom-diagnostic', auth: 'stubbed', vault: 'none', profileRemoved, serverClosed: !server.listening, ...result })}\n`,
);
