...
function renderSavedFormMatrixHTML(kind) {
  if (
    ...
    throw new Error('saved_matrix_kind_invalid');
  if (kind === 'username_first')
  ...
  if (kind === 'frame_login')
    return `<!doctype html><main><form method="post" action="/submitted" onsubmit="event.preventDefault();fetch('/submitted',{method:'POST'})"><input id="username" autocomplete="username"><input id="password" type="password" autocomplete="current-password"><input id="unrelated" value="unchanged"></form></main>`;
  if (kind === 'same_origin_frame')
  ...
  if (kind === 'two_same_origin_frames')
...
exports.runSavedFormMatrix = async ({
  context,
  worker,
  realPanel,
  targetName,
  ...
  assert(
    context && worker && realPanel && typeof parentOrigin === 'string',
    'saved_matrix_context_missing',
  );
  assert(
  ...
  assert(
    ...
    'saved_matrix_parent_origin_invalid',
  );
  const evidence = {
    usernameFirstFilled: false,
    lateSpaFilled: false,
    sameOriginFrameFilled: false,
    parentPointerStaleControlRemoved: false,
    sameUrlSiblingBOnlyFilled: false,
    crossOriginFrameFilled: false,
    childFramesReady: false,
    quietNoOverlay: false,
  ...
  let primaryFailure;
  let focusedFixture;
  const url = (kind, origin = parentOrigin) =>
    `${origin}/saved-matrix/${kind}?case=${crypto.randomUUID()}`;
  const tabFor = async (expected) => {
    for (let i = 0; i < READY_ATTEMPTS; i += 1) {
      const id = await worker.evaluate(
        async (u) => (await chrome.tabs.query({})).find((tab) => tab.url === u)?.id,
        expected,
      );
  ...
  const activeNormalWindow = async (tabId) =>
  ...
  const childReady = async (tabId, expectedUrl, expectedCount = 1) => {
    for (let attempt = 0; attempt < READY_ATTEMPTS; attempt += 1) {
      const ready = await worker.evaluate(
        async ({ id, expected, count }) => {
          const frames = (await chrome.webNavigation.getAllFrames({ tabId: id })).filter(
            (frame) => frame.url === expected,
          );
          ...
          const results = await Promise.all(
            frames.map(async (frame) => {
              const injected = await chrome.scripting.executeScript({
                ...
                func: () => window.__matrx_generation_target_registry__ !== undefined,
              });
  ...
  const focusTop = async ({ page, tabId }, selector) => {
    ...
    assert(
      await page.evaluate(
        (id) => document.hasFocus() && document.activeElement?.id === id,
        selector.slice(1),
      ),
      'saved_matrix_top_document_focus_invalid',
    );
    focusedFixture = page;
  };
  const focusChild = async ({ page, tabId }, frameElement, expectedUrl, selector) => {
    ...
    assert(
      page
        ...
        .some(
          (candidate) =>
            candidate.url() === expectedUrl && candidate.parentFrame() === page.mainFrame(),
        ),
      'saved_matrix_child_frame_missing',
    );
    ...
    assert(
      await frame
        .locator(selector)
        .evaluate((element) => document.hasFocus() && document.activeElement === element),
      'saved_matrix_child_document_focus_invalid',
    );
    assert(
      (await page.locator(frameElement).count()) === 1,
      'saved_matrix_child_frame_element_missing',
    );
  ...
  const stableQuiet = async (frame) => {
    ...
    const observe = async () => {
      assert(
        (await frame.locator('#matrx-inline-login-suggestion').count()) === 0,
        'saved_matrix_inline_overlay_present',
      );
    };
    await observe();
  ...
  const panelFillAvailable = async () => {
    ...
    try {
      await realPanel.waitFor(`!!(${selector})`, true, 15000);
    } catch {
      throw new Error('saved_matrix_panel_fill_unavailable');
    }
    return selector;
  };
  const focusedFieldsMatch = async () => {
    assert(focusedFixture, 'saved_matrix_focused_fixture_missing');
    return focusedFixture.locator('body').evaluate(
      (body, expected) => {
        const fields = [
          ...
          .map(([selector, value]) => ({
            input: body.ownerDocument.querySelector(selector),
            value,
          }))
          .filter(({ input }) => !!input);
  ...
  const fill = async () => {
    ...
    try {
      ...
      if (focusedFixture)
        evidence.fillFailure.fields = await focusedFixture.locator('body').evaluate(
          (body, expected) => {
            const user = body.ownerDocument.querySelector('#username');
            const pass = body.ownerDocument.querySelector('#password');
            return {
              usernamePresent: !!user,
              usernameMatches: user?.value === expected.username,
              passwordPresent: !!pass,
              passwordMatches: pass?.value === expected.password,
            };
          },
          { username, password },
        );
  ...
  const noActionableFill = async () => {
    await verifyRealVaultPanel();
    try {
      await realPanel.waitFor(`!(${fillButton(targetName)})`, true, 15000);
    } catch {
      throw new Error('saved_matrix_stale_fill_remained_actionable');
    }
  };
  const valuesAreUntouched = async (frame) => {
    assert(
      (await frame.locator('#username').inputValue()) === '',
      'saved_matrix_stale_username_changed',
    );
    assert(
      (await frame.locator('#password').inputValue()) === '',
      'saved_matrix_stale_password_changed',
    );
    assert(
      (await frame.locator('#unrelated').inputValue()) === 'unchanged',
      'saved_matrix_stale_unrelated_changed',
    );
  };
  try {
    checkpoint('saved_matrix_username_first');
    const first = await open('username_first');
    await focusTop(first, '#username');
    await stableQuiet(first.page);
    checkpoint('saved_matrix_username_only_fill');
    await fill();
    assert(
      (await first.page.locator('#username').inputValue()) === username,
      'saved_matrix_username_first_username',
    );
    assert(
      (await first.page.locator('#password').count()) === 0,
      'saved_matrix_username_first_automatic_continue',
    );
    checkpoint('saved_matrix_explicit_continue');
    await first.page.locator('#continue').click();
    await focusTop(first, '#password');
    await stableQuiet(first.page);
    checkpoint('saved_matrix_second_step_fill');
    await fill();
    assert(
      (await first.page.locator('#password').inputValue()) === password,
      'saved_matrix_username_first_password',
    );
    assert(
      (await first.page.locator('#username').inputValue()) === username &&
        (await first.page.locator('#decoy').inputValue()) === 'unchanged',
    ...
    await late.page.locator('#mount').click();
    await focusTop(late, '#password');
    await stableQuiet(late.page);
    await fill();
    assert(
      (await late.page.locator('#username').inputValue()) === username &&
        (await late.page.locator('#password').inputValue()) === password,
      'saved_matrix_late_spa_values',
    );
    assert(
      (await late.page.locator('#decoy').inputValue()) === 'unchanged',
    ...
    await childReady(framed.tabId, sameChildUrl);
    evidence.childFramesReady = true;
    const sameFrame = await focusChild(framed, '#login-frame', sameChildUrl, '#password');
    await stableQuiet(sameFrame);
    await fill();
    assert(
      (await sameFrame.locator('#username').inputValue()) === username &&
        (await sameFrame.locator('#password').inputValue()) === password,
      'saved_matrix_frame_values',
    );
    assert(
      (await framed.page.locator('#decoy').inputValue()) === 'unchanged' &&
        (await sameFrame.locator('#unrelated').inputValue()) === 'unchanged',
      'saved_matrix_frame_unrelated_changed',
    );
    ...
    await childReady(siblings.tabId, siblingUrl, 2);
    evidence.childFramesReady = true;
    const siblingFrames = siblings.page
      .frames()
      .filter(
        (candidate) =>
          candidate.url() === siblingUrl && candidate.parentFrame() === siblings.page.mainFrame(),
      );
    const siblingA = siblings.page.frameLocator('#frame-a');
    const siblingB = siblings.page.frameLocator('#frame-b');
    childDocuments.add(siblingA);
    childDocuments.add(siblingB);
    assert(siblingFrames.length === 2, 'saved_matrix_same_url_siblings_missing');
    await focusChild(siblings, '#frame-a', siblingUrl, '#password');
    await panelFillAvailable();
    await siblings.page.bringToFront();
    await focusOwnedBrowser(siblings.tabId);
    assert(
      await activeNormalWindow(siblings.tabId),
      'saved_matrix_owned_normal_window_not_focused',
    );
    await siblings.page.locator('#parent-pointer').click();
    assert(
      await siblings.page.evaluate(
        () =>
          document.hasFocus() &&
          window.__parentPointerClicks === 1 &&
          window.__parentDecoyFocusCount === 0,
      ),
      'saved_matrix_parent_pointer_focus_invalid',
    );
    await stableQuiet(siblings.page);
    await noActionableFill();
    await Promise.all([stableQuiet(siblingA), stableQuiet(siblingB)]);
    await valuesAreUntouched(siblingA);
    await valuesAreUntouched(siblingB);
    assert(
      (await siblings.page.locator('#parent-decoy').inputValue()) === 'unchanged',
      'saved_matrix_parent_focus_collateral_write',
    );
    evidence.parentPointerStaleControlRemoved = true;
    await focusChild(siblings, '#frame-a', siblingUrl, '#password');
    await panelFillAvailable();
    await focusChild(siblings, '#frame-b', siblingUrl, '#password');
    await stableQuiet(siblingB);
    await fill();
    await Promise.all([stableQuiet(siblingA), stableQuiet(siblingB)]);
    await valuesAreUntouched(siblingA);
    assert(
      (await siblingB.locator('#username').inputValue()) === username &&
        (await siblingB.locator('#password').inputValue()) === password &&
        (await siblingB.locator('#unrelated').inputValue()) === 'unchanged',
      'saved_matrix_sibling_b_fill_invalid',
    );
    assert(
      (await siblings.page.locator('#parent-decoy').inputValue()) === 'unchanged',
      'saved_matrix_sibling_parent_changed',
    );
    ...
    const permissionsPresent = await worker.evaluate(
      async ({ top, child }) =>
        (
          await Promise.all(
            [top, child].map((origin) => chrome.permissions.contains({ origins: [`${origin}/*`] })),
          )
        ).every(Boolean),
      { top: crossOrigin, child: parentOrigin },
    );
    assert(permissionsPresent, 'saved_matrix_cross_origin_permission_missing');
    await childReady(crossed.tabId, crossChildUrl);
    evidence.childFramesReady = true;
    const crossFrame = await focusChild(crossed, '#cross-frame', crossChildUrl, '#password');
    await stableQuiet(crossFrame);
    await fill();
    assert(
      (await crossFrame.locator('#username').inputValue()) === username &&
        (await crossFrame.locator('#password').inputValue()) === password &&
        (await crossFrame.locator('#unrelated').inputValue()) === 'unchanged',
      'saved_matrix_cross_origin_values',
    );
    assert(
      (await crossed.page.locator('#parent-decoy').inputValue()) === 'unchanged',
      'saved_matrix_cross_origin_parent_changed',
    );
  ...
  assert(
    evidence.usernameFirstFilled &&
      evidence.lateSpaFilled &&
      evidence.sameOriginFrameFilled &&
      evidence.parentPointerStaleControlRemoved &&
      evidence.sameUrlSiblingBOnlyFilled &&
      evidence.crossOriginFrameFilled &&
      evidence.childFramesReady &&
      evidence.quietNoOverlay &&
      evidence.noWebsiteSubmission &&
      evidence.pagesClosed,
    'saved_matrix_incomplete',
  );
  return evidence;
};
