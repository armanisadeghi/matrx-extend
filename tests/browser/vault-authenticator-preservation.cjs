...
const stableMetadata = (entry) => {
  ...
  assert(
    typeof metadata.credential_item_id === 'string' && metadata.credential_item_id.length > 0,
    'authenticator_item_id_missing',
  );
  assert(
    typeof metadata.seed_field_id === 'string' && metadata.seed_field_id.length > 0,
    'authenticator_seed_field_missing',
  );
  assert(
    metadata.digits === 8 && metadata.period === 30 && metadata.enabled === true,
    'authenticator_parameters_invalid',
  );
...
const runCanonical = (script, payload) =>
  new Promise((resolve, reject) => {
    const child = spawn(AIDREAM_PYTHON, ['-c', script], {
    ...
    child.once('error', () => {
      ...
      reject(new Error('canonical_totp_unavailable'));
    });
...
const canonicalVerifier = ({ seed, code, metadata, startedAt, endedAt }) =>
  runCanonical(
    [
      'import json, sys',
...
const exerciseCanonicalPrimitive = ({ seed, metadata, forTime }) =>
  runCanonical(
    [
      'import json, sys',
      'from aidream.services.authenticator.otp import decode_seed, generate_code',
...
const routeStatus = (response) =>
  Number.isInteger(response?.status) ? response.status : 'transport';

function createAuthenticatorPreservation({
  request,
  markCleanupObligation,
  journal = () => {},
  now = () => Date.now() / 1000,
  createEnrollmentInput = createEnrollment,
  verifyCode = canonicalVerifier,
  maxCodeRequestSeconds = 5,
}) {
  ...
  assert(
    ...
    'authenticator_controls_missing',
  );

  const journalRoute = (method, response) => {
    try {
      journal({ method, route: 'authenticator', status: routeStatus(response) });
    } catch {
      throw new Error('authenticator_journal_failed');
    }
  };
  ...
  const reconcile = async (credentialItemId) =>
    call('GET', `/${encodeURIComponent(credentialItemId)}`);

  const validateCode = async (handle, response) => {
    const secret = privateState.get(handle);
    assert(secret, 'authenticator_handle_unknown');
    assert(
      response?.status === 200 && typeof response.body?.code === 'string',
      'authenticator_code_unavailable',
    );
    const endedAt = now();
    assert(
      endedAt - secret.codeStartedAt <= Math.min(maxCodeRequestSeconds, secret.metadata.period),
      'authenticator_code_window_unbounded',
    );
    const matches = await verifyCode({
      seed: secret.seed,
      code: response.body.code,
      metadata: secret.metadata,
      startedAt: secret.codeStartedAt,
      endedAt,
    });
  ...
  const enroll = async ({ credentialItemId }) => {
    assert(
      typeof credentialItemId === 'string' && credentialItemId.length > 0,
      'authenticator_target_missing',
    );
    const handle = {
      credentialItemId,
      cleanupObligation: true,
      enrolled: false,
    ...
    assert(
      typeof enrollment?.seed === 'string' && typeof enrollment?.enrollmentInput === 'string',
      'authenticator_enrollment_input_invalid',
    );
    const secret = privateState.get(handle);
    secret.seed = enrollment.seed;
    const posted = await call('POST', '/enroll', {
      credential_item_id: credentialItemId,
      enrollment_input: enrollment.enrollmentInput,
    });
    let entry;
      ...
      if (reconciled.status === 200) entry = reconciled.body;
        ...
        handle.failure =
          reconciled.status === 404
            ? 'enrollment_absent_after_ambiguity'
            : 'enrollment_custody_unresolved';
        return handle;
      }
    }
  ...
  const verify = async (handle) => {
    ...
    assert(
      sameMetadata(secret.metadata, stableMetadata(after.body)),
      'authenticator_metadata_changed',
    );
    secret.codeStartedAt = now();
    await validateCode(
      handle,
      await call('GET', `/${encodeURIComponent(handle.credentialItemId)}/code`),
    );
  ...
  const cleanup = async (handle) => {
    ...
    const deleted = await call(
      'DELETE',
      `/${encodeURIComponent(handle.credentialItemId)}`,
      undefined,
      handle,
    );
