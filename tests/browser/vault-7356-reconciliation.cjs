...
const FRESH_NONOWNED_BASELINE_SHA256 = '6f837f99946929f30802980e6149050464b140eea0f6e81e088e852a3e9eda1f';
...
const FAILED_PROOF_PATH = path.join(
  REPO_ROOT, '.matrx/realbrowser-vault/save-update-headless', FAILED_RUN_ID, 'proof.json',
);
const RECOVERY_PATH = path.join(REPO_ROOT, '.matrx/task1-active/recovery-7356-result.json');
const LEASE_RETIREMENT_PATH = path.join(REPO_ROOT, '.matrx/task1-active/lease-retirement-7356.json');
...
function assertExactPaths(failedProofPath, recoveryPath, leaseRetirementPath) {
  ...
  assert.equal(path.resolve(leaseRetirementPath), LEASE_RETIREMENT_PATH, '7356_lease_retirement_path_mismatch');
}
...
function assertFailedRecord(record) {
  ...
  assert.equal(exactSet(record?.ownedFixtureIds, OWNED_FIXTURE_IDS), true, '7356_failed_owned_fixture_ids_mismatch');
  assert.equal(record?.cleanup?.localAuthLogoutStatus, 504, '7356_old_logout_status_mismatch');
  assert.equal(record?.cleanup?.remoteAuthRevocationStatus, 504, '7356_old_remote_revocation_status_mismatch');
...
function assertRecoveryRecord(record) {
  ...
  assert.equal(record?.failedProofSha256, FAILED_PROOF_SHA256, '7356_recovery_failed_hash_mismatch');
  ...
  assert.equal(record?.oldRemoteRevocation, 'unconfirmed_504_no_old_bearer_retained', '7356_old_revocation_must_remain_unconfirmed');
  ...
  assert.equal(record?.adapter?.route, 'local_canonical_authmiddleware', '7356_cleanup_adapter_route_mismatch');
  assert.equal(record?.adapter?.provenance, 'local_router_and_service_hash_pinned', '7356_cleanup_adapter_provenance_mismatch');
  assert.equal(record?.adapter?.sourceSha256?.router, ROUTER_SHA256, '7356_cleanup_router_hash_mismatch');
  assert.equal(record?.adapter?.sourceSha256?.service, SERVICE_SHA256, '7356_cleanup_service_hash_mismatch');
  ...
  assert.equal(Array.isArray(record?.freshBaseline) && record.freshBaseline.length === 35
    ...
    && sha256(JSON.stringify(record.freshBaseline)) === FRESH_NONOWNED_BASELINE_SHA256, true,
  ...
  assert.equal(record?.adapter?.receiptCount, OWNED_FIXTURE_IDS.length, '7356_cleanup_receipt_count_mismatch');
  assert.equal(Array.isArray(attempts) && attempts.length === OWNED_FIXTURE_IDS.length
    && new Set(attempts.map(entry => entry?.id)).size === OWNED_FIXTURE_IDS.length
    && attempts.every(entry => OWNED_FIXTURE_IDS.includes(entry?.id)
      && entry?.initialGetStatus === 200 && entry?.deleteStatus === 204
      && entry?.finalGetStatus === 404 && entry?.terminal === 'deleted_and_missing'), true,
...
function assertLeaseRetirementRecord(record) {
  ...
  assert.equal(record?.kind, 'reviewed_exact_quarantine_retirement', '7356_lease_retirement_kind_mismatch');
  ...
  assert.equal(record?.owner?.nonce, RETIRED_OWNER_NONCE, '7356_lease_retirement_owner_nonce_mismatch');
  assert.equal(record?.ownerSha256, RETIRED_OWNER_SHA256, '7356_lease_retirement_owner_hash_mismatch');
  assert.equal(record?.failedProofSha256, FAILED_PROOF_SHA256, '7356_lease_retirement_failed_hash_mismatch');
  assert.equal(record?.recoverySha256, RECOVERY_SHA256, '7356_lease_retirement_recovery_hash_mismatch');
  assert.equal(record?.failedRunRemainsFailed, true, '7356_lease_retirement_failed_verdict_missing');
  assert.equal(record?.oldRemoteRevocation, 'unconfirmed_504_no_old_bearer_retained', '7356_lease_retirement_old_revocation_overclaimed');
  assert.equal(record?.ownedFixturesReconciled, true, '7356_lease_retirement_fixtures_missing');
  assert.equal(record?.currentNonOwnedInventoryPreserved, true, '7356_lease_retirement_nonowned_inventory_missing');
...
function assert7356RecoveryAdmission({
  failedProofPath, failedRaw, recoveryPath, recoveryRaw, leaseRetirementPath, leaseRetirementRaw,
}) {
  ...
  assert.equal(sha256(leaseRetirementRaw), LEASE_RETIREMENT_SHA256, '7356_lease_retirement_hash_changed');
