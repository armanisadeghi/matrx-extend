      ...
      if (this.clicks === 1 || this.clicks === 3) {
        for (const listener of listeners) {
          listener('Fetch.requestPaused', {
            ...
            request: { method: 'GET', url: 'https://server.example.test/api/vault/items?principal_type=user' },
          });
        }
      }
    },
    async waitFor() { await new Promise((resolve) => setImmediate(resolve)); },
  };
  const proof = {};
  const result = await runVaultListTransportRecoveryChecks({
    context: {}, realPanel: panel, minimumOwnListRows: 1, apiOrigin: 'https://server.example.test',
    exactPanelDocumentUrl: 'chrome-extension://owned/sidepanel.html', getVaultWriteCount: () => 0,
    snapshotOwnedReceiptState: () => [], assert: (condition, code) => assert.ok(condition, code),
    proof, verifyRealVaultPanel: async () => {},
  });
