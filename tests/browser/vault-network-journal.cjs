...
function classifyVaultRequest({ url, method, apiOrigin }) {
  ...
  try {
    ...
    return { inVault: false, mutation: false, metadataRead: false, itemCreate: false };
  }
  ...
  return {
    inVault,
    mutation: inVault && VAULT_METHODS.has(normalizedMethod) && !metadataRead,
    metadataRead,
    itemCreate: inVault && normalizedMethod === 'POST' && parsed.pathname === ITEMS_PATH,
    method: normalizedMethod,
    pathname: parsed.pathname,
  };
}

function createVaultNetworkJournal({
  cdp,
  apiOrigin,
  ownedLocalOrigin,
  onVaultRequest = () => {},
}) {
  if (!cdp?.send || !cdp?.on || !cdp?.off || typeof apiOrigin !== 'string')
    throw new Error('vault_network_cdp_required');
  let started = false,
    disposing = false,
    disposed = false,
    observerError = false,
    ...
    disposePromise,
    cleanupPhase = 'idle',
    lastSendFailureClass = 'none';
  let mutations = 0,
    metadata = 0,
    pageRequestCount = 0,
    ownedLocalPageRequestCount = 0,
            ...
            : method === 'Target.detachFromTarget'
              ...
              : method === 'Runtime.runIfWaitingForDebugger'
                ...
                : method === 'Network.enable'
                  ...
                  : 'other';
      fail();
      throw new Error('vault_network_protocol_failed');
    }
  };
  ...
  const sessionsFor = (targetId) =>
    [...(targetSessions.get(targetId) ?? [])].map((id) => targets.get(id)).filter(Boolean);
  const observingFor = (targetId) => {
    const canonical = targets.get(canonicalSessions.get(targetId));
    return canonical?.setupDone && canonical.enabled && !canonical.detached && !canonical.detaching
      ? canonical
      : undefined;
  };
  const freshEpoch = (target) => {
    boundTarget = target.targetId;
    boundSession = target.sessionId;
    requestSeen = false;
    responseSeen = false;
  ...
  const removeTarget = (target) => {
    ...
    if (canonicalSessions.get(target.targetId) === target.sessionId)
      canonicalSessions.delete(target.targetId);
    if (target.sessionId === boundSession) {
      boundSession = null;
      requestSeen = false;
      responseSeen = false;
    }
  };
  const waitForAttached = (sessionId, targetId) =>
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        attachedWaiters.delete(sessionId);
        fail();
        reject(new Error('vault_network_panel_attach_timeout'));
      }, ATTACH_TIMEOUT_MS);
  ...
  const settleWaiter = (sessionId, target) => {
    ...
    if (waiter.targetId !== target.targetId) {
      fail();
      waiter.reject(new Error('vault_network_panel_attach_mismatch'));
      return;
    }
    waiter.resolve(target);
  };
  const waitForDetached = (target) =>
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        detachWaiters.delete(target.sessionId);
        fail();
        reject(new Error('vault_network_redundant_detach_timeout'));
      }, DETACH_TIMEOUT_MS);
  ...
  const reconcileTarget = (target) => {
    ...
    if (!canonical) {
      canonicalSessions.set(target.targetId, target.sessionId);
      return;
    }
  ...
  const setup = async (params, manual) => {
    ...
    try {
      ...
      if (target.paused) {
        if (!target.enabled) enableSuccessBeforeResume = false;
        await resume(target);
      }
  ...
  const onDetached = (params) => {
    ...
    if (waiter) {
      ...
      clearTimeout(waiter.timer);
      fail();
      waiter.reject(new Error('vault_network_panel_attach_detached'));
    }
    ...
    if (detachWaiter) {
      detachWaiters.delete(sessionId);
      clearTimeout(detachWaiter.timer);
      detachWaiter.resolve();
    }
    removeTarget(target);
    const replacement = sessionsFor(target.targetId).find(
      (candidate) =>
        candidate.setupDone && candidate.enabled && !candidate.detached && !candidate.detaching,
    );
  ...
  const onRequest = (params, sessionId) => {
    ...
    if (
      ...
      disposed ||
      target.detaching ||
      canonicalSessions.get(target.targetId) !== sessionId
    )
      return;
    pageRequestCount += 1;
    let origin = '';
    ...
    const entry = {
      ordinal: (old?.ordinal ?? 0) + 1,
      items: c.inVault && c.method === 'GET' && c.pathname === ITEMS_PATH,
      c,
    };
    target.requests.set(params.requestId, entry);
    promote(target);
    if (!c.inVault) return;
    ...
    try {
      onVaultRequest({
        ...
        itemCreate: c.itemCreate,
        mutation: c.mutation,
        metadataRead: c.metadataRead,
      });
    } catch {
      fail();
    }
  ...
  const onResponse = (params, sessionId) => {
    const target = targets.get(sessionId),
      entry = target?.requests.get(params.requestId);
    if (
      !target ||
      !entry?.items ||
      disposed ||
      ...
      return;
    if (
      Number.isInteger(params.response?.status) &&
  ...
  const bind = async (targetId) => {
    if (!started || disposing || disposed || typeof targetId !== 'string' || !targetId)
      throw new Error('vault_network_panel_bind_unavailable');
    await drain();
    const already = observingFor(targetId);
    if (already) {
      freshEpoch(already);
      return { targetId, sessionId: already.sessionId };
    }
    const attached = await send('Target.attachToTarget', { targetId, flatten: true });
    const sessionId = attached?.sessionId;
    if (typeof sessionId !== 'string' || !sessionId) {
      fail();
      throw new Error('vault_network_panel_attach_missing');
    }
    let target = targets.get(sessionId);
    if (target) {
      if (target.targetId !== targetId) {
        fail();
        throw new Error('vault_network_panel_attach_mismatch');
      }
      target.manual = true;
    } else {
    ...
    await drain();
    if (target.targetId !== targetId || !target.setupDone || !target.enabled) {
      fail();
      throw new Error('vault_network_panel_setup_failed');
    }
    await drain();
  ...
  return {
    async start() {
      ...
      try {
        ...
        throw new Error('vault_network_autoattach_failed');
      }
    },
    ...
    async settle() {
      await drain();
      await Promise.allSettled([...binds.values()]);
    },
    snapshot() {
      ...
      return {
        boundTargetAttached: canonical.length > 0,
        boundTargetSessionCount: bound.length,
        boundTargetCountingSessionCount: canonical.length,
        boundTargetRequestCount: boundRequests.length,
        boundTargetVaultRequestCount: boundRequests.filter((entry) => entry.c.inVault).length,
        boundTargetItemsAnyOriginCount: boundRequests.filter(
          (entry) => entry.c.pathname === ITEMS_PATH,
        ).length,
        journalSemanticVersion: 2,
        vaultMutationRequests: mutations,
        vaultMetadataReadRequests: metadata,
        pageRequestCount,
        ownedLocalPageRequestCount,
        enableSuccessBeforeResume,
        panelItemsReadRequestSeen: requestSeen,
        panelItemsReadResponse2xxSeen: responseSeen,
        observerError,
        cleanupPhase,
        ...
        transportCloseStatus: typeof cdp.closeStatus === 'string' ? cdp.closeStatus : 'unknown',
      };
    },
    async assertCoverage() {
      await drain();
      await Promise.all([...binds.values()]);
    ...
    async dispose() {
      ...
      disposePromise = (async () => {
        ...
        cleanupPhase = 'draining_setup';
        await drain();
        await Promise.allSettled([...binds.values()]);
        cleanupPhase = 'detaching_manual';
        for (const target of [...targets.values()])
          if (target.manual && !target.detached)
            await detachTarget(target).catch(() => {
              cleanup = true;
            });
        ...
        disposed = true;
        cleanupPhase = cleanup || observerError || cdp.fatal ? 'failed' : 'complete';
