...
const secondPortUrls = fixtureUrlsForParent(
  'http://127.0.0.1:43128/login',
...
(async () => {
  ...
  await assert.rejects(
    () =>
      runSavedLoginChecks({
        ...
        worker: {},
        realPanel: {},
        targetName: 'Receipt-owned login',
    ...
    () =>
      runSavedLoginChecks({
        context: {
          newPage: async () => ({
            goto: async () => {
              throw new Error('primary_navigation_failure');
            },
            isClosed: () => {
              isClosedChecks += 1;
              return false;
            ...
            close: async () => {
              closeAttempts += 1;
              throw new Error('page_close_failure');
            },
          }),
        },
        worker: {},
  ...
  assert.equal(
    cleanupProof.savedLoginFill.pagesClosed,
    false,
    'failed page cleanup was reported as closed',
  );
  assert.equal(
    cleanupProof.savedLoginFill.pageCleanupFailure,
    true,
    'failed page cleanup was not surfaced in evidence',
  );
  process.stdout.write(
    'PASS: saved-login fixture uses the parent exact origin and refuses mismatches before page creation\n',
  );
})().catch((error) => {
