...
const base = (overrides = {}) => ({
  context: {
    newPage: async () => {
      throw new Error('forced_body_failure');
    },
  },
  worker: {},
  panel: {},
...
(async () => {
    ...
    await assert.rejects(
      runGeneratorChecks(
        base({
          ...
          proof: {
            clipboardIsolation: { runtime: 'Chrome/152.0.0.0', ownedHeadlessProcess: true },
          },
        }),
      ),
  ...
  await assert.rejects(
    runGeneratorChecks(base({ proof: earlyFailureProof })),
    /forced_body_failure/,
  );
      ...
      base({
        ...
        context: {
          newPage: async () => ({
            ...
            isClosed: () => false,
            close: async () => {
              throw new Error('forced_page_close_failure');
            },
          }),
        },
      }),
  ...
  http.createServer = (...args) => {
    ...
    if (serverIndex === 2) {
      ...
      server.close = (callback) =>
        realClose((error) => callback(error || new Error('forced_top_close_failure')));
    }
    return server;
  };
  ...
  try {
    await assert.rejects(
      runGeneratorChecks(base({ proof: serverFailureProof })),
      /generator_fixture_cleanup_failed/,
    );
  ...
  process.stdout.write(
    'PASS: generator refuses before fixture custody and independently proves every cleanup branch\n',
  );
})().catch((error) => {
