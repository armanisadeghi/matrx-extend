...
exports.acquireVaultAcceptanceLease = async ({ runId, kind, lockPath = DEFAULT_LOCK }) => {
  if (typeof runId !== 'string' || typeof kind !== 'string') throw new Error('vault_lease_identity_missing');
  ...
  try {
    await fs.writeFile(ownerPath, JSON.stringify({ schema: 1, pid: process.pid, runId, kind, nonce, acquiredAt: new Date().toISOString() }) + '\n', { mode: 0o600, flag: 'wx' });
  } catch (error) {
  ...
  return {
    ...
    async release() {
      ...
      if (!stat.isDirectory() || stat.isSymbolicLink()) throw new Error('vault_lease_directory_changed');
      const owner = JSON.parse(await fs.readFile(ownerPath, 'utf8'));
      if (owner.nonce !== nonce || owner.pid !== process.pid || owner.runId !== runId) throw new Error('vault_lease_owner_changed');
