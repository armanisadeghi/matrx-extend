...
function windowSwitchUiCleared(state) {
  ...
  return (
    ...
    state.actionLabel === 'generate' &&
    state.actionReady === true
  );
}
function hasIsolatedHeadlessClipboardProof(proof) {
  return (
    proof?.clipboardIsolation?.runtime === 'Chrome/153.0.8010.12' &&
...
exports.runGeneratorChecks = async ({
  ...
  displayMode,
  panelCloseLifecycleMode = false,
  workerRestartLifecycleMode = false,
  windowSwitchLifecycleMode = false,
  reopenPanelFromAction,
  refreshWorker,
  isolatedHeadlessClipboard = false,
}) => {
  assert(
    displayMode === 'HEADLESS_NO_CLIPBOARD' || displayMode === 'HEADED',
    'generator_display_mode_required',
  );
  let child;
  let top;
  let page;
  ...
  try {
    ...
    if (isolatedHeadlessClipboard) {
      ...
      assert(
        ...
        'generator_isolated_clipboard_not_fresh',
      );
    ...
    for (let focusAttempt = 0; focusAttempt < 5; focusAttempt++) {
      ...
      evidence.focusDiagnostic = await worker.evaluate(async (id) => {
        ...
        return {
          ...
          type: focused.type,
          selectedWindowMatches: focused.id === tab.windowId,
          tabActive: tab.active,
        };
      }, tabId);
      if (
        evidence.focusDiagnostic.focused &&
        evidence.focusDiagnostic.selectedWindowMatches &&
        evidence.focusDiagnostic.tabActive
      )
        break;
      await wait(250);
    }
    assert(
      evidence.focusDiagnostic.focused &&
        evidence.focusDiagnostic.selectedWindowMatches &&
        evidence.focusDiagnostic.tabActive,
      'generator_owned_window_not_focused',
    );
    ...
    const openGeneratorConnection = async () => {
      ...
      assert(
        typeof next === 'string' && /^[a-f0-9]{36}$/.test(next),
        'generator_panel_port_handshake_missing',
      );
      return next;
    };
    ...
    const request = (payload) => {
      ...
      return panel.evaluate(
        `chrome.runtime.sendMessage(${JSON.stringify({ __matrxCredentialGeneration: true, connectionId, ...payload })})`,
      );
    };
    ...
    const unchanged = async (frame) =>
      frame.evaluate(
        () =>
          document.querySelector('#new').value === '' &&
          document.querySelector('#confirm').value === '' &&
          document.querySelector('#current').value === 'owned-current-fixture',
      );
    const matches = async (frame) =>
      frame.evaluate(
        (value) =>
          document.querySelector('#new').value === value &&
          document.querySelector('#confirm').value === value &&
          document.querySelector('#current').value === 'owned-current-fixture',
        synthetic,
      );
    ...
    try {
      evidence.senderDiagnostic = await panel.evaluate(
        `chrome.runtime.sendMessage(${JSON.stringify({ vaultCanarySenderProbe: probeId })})`,
      );
    } finally {
    ...
    assert(
      first?.status === 'ready' && first.offers.length === 3,
      'generator_exact_frames_not_offered',
    );
    const topOffer = first.offers.find(
      (offer) => offer.frameId === 0 && offer.origin === new URL(top.url).origin,
    );
    const childOffer = first.offers.find(
      (offer) => offer.frameId !== 0 && offer.origin === new URL(child.url).origin,
    );
    assert(topOffer && childOffer && embedded(), 'generator_frame_origin_binding_missing');
    assert(
      first.offers.every((offer) => offer.fieldCount === 2),
      'generator_current_password_offered',
    );
    ...
    assert(
      (await discard([discardedOffer.id]))?.status === 'discarded',
      'generator_discard_refused',
    );
    assert(
      (await use(discardedOffer.id))?.status === 'stale',
      'generator_discarded_offer_survived',
    );
    ...
    const selectedChild = second?.offers?.find(
      (offer) => offer.frameId !== 0 && offer.origin === new URL(child.url).origin,
    );
    ...
    const sameOriginOffer = sameOriginDiscovery?.offers?.find(
      (offer) => offer.frameId !== 0 && offer.origin === new URL(top.url).origin,
    );
    const sameOriginFrame = page
      .frames()
      .find((frame) => frame.url() === new URL('/embedded', top.url).href);
    assert(sameOriginOffer && sameOriginFrame, 'generator_same_origin_frame_missing');
    assert(
      (await use(sameOriginOffer.id))?.status === 'filled',
      'generator_same_origin_fill_refused',
    );
    ...
    assert(
      (await use(replacedOffer.id))?.status === 'refused_unchanged',
      'generator_replacement_not_refused',
    );
    ...
    evidence.navigationDiscovery = {
      ...
      offerCount: fourth?.offers?.length ?? 0,
      focus: await worker.evaluate(async (id) => {
        const tab = await chrome.tabs.get(id);
        const win = await chrome.windows.get(tab.windowId);
        return { focused: win.focused, active: tab.active };
      }, tabId),
    };
    ...
    await page.reload();
    assert(
      (await use(oldDocumentOffer.id))?.status === 'stale',
      'generator_navigation_offer_survived',
    );
    ...
    assert(
      !unsafeDiscovery?.offers?.some((offer) => offer.frameId === 0),
      'generator_get_form_offered',
    );
    ...
    await page.evaluate((origin) => {
      document.querySelector('form').action = origin + '/receive';
    }, new URL(child.url).origin);
    assert(
      (await use(actionOffer.id))?.status === 'refused_unchanged',
      'generator_changed_action_not_refused',
    );
    ...
    assert(
      (await use(rollbackOffer.id))?.status === 'rolled_back',
      'generator_action_reaction_not_rolled_back',
    );
    ...
    const panelIdentity = async () => {
      const context = await worker.evaluate(() =>
      ...
      assert(
        typeof panel.targetId === 'string' &&
          ...
          context.length === 1 &&
          typeof context[0].documentId === 'string' &&
          context[0].documentId.length > 0,
        'lifecycle_panel_identity_unavailable',
      );
    ...
    const generateUiOffer = async () => {
      ...
      if (
        await panel.evaluate(
          `(${lifecycleButton('Password generator')})?.getAttribute('aria-expanded') !== 'true'`,
        )
      )
      ...
      await panel.waitFor(
        `!!(${lifecycleSection})?.querySelector('[aria-label="Reveal generated value"]') && (${lifecycleSection})?.innerText.includes('Generated.')`,
      );
      const state = await uiCandidateState();
      assert(
        state.regionPresent && state.generatedCandidatePresent && state.offerPresent,
        'lifecycle_ui_candidate_missing',
      );
    ...
    const freshUiGenerateUse = async (code) => {
      ...
      if (
        ...
        await panel.click(lifecycleButton('Password generator'));
      await panel.waitFor(
        `!!(${lifecycleButton('Generate')}) && !(${lifecycleButton('Generate')}).disabled`,
      );
      await panel.click(lifecycleButton('Generate'));
      await panel.waitFor(
        `!!(${lifecycleSection})?.querySelector('[aria-label="Reveal generated value"]') && (${lifecycleSection})?.innerText.includes('Generated.')`,
      ...
      await panel.click(
        `(${lifecycleSection}).querySelector('[aria-label="Reveal generated value"]')`,
      );
      const uiValue = await panel.evaluate(
        `(${lifecycleSection}).querySelector('code')?.textContent`,
      );
      assert(
        typeof uiValue === 'string' && uiValue.length > 0 && uiValue !== '••••••••••••••••••••••••',
        'lifecycle_ui_reveal_missing',
      );
      await panel.click(
        `(${lifecycleSection}).querySelector('[aria-label="Hide generated value"]')`,
      );
      await panel.click(lifecycleButton('Use'));
      await panel.waitFor(
        `!(${lifecycleSection})?.querySelector('code') && /Filled/.test((${lifecycleSection})?.innerText ?? '')`,
      );
      const uiFilled = await page.evaluate(
        (value) =>
          document.querySelector('#new').value === value &&
          document.querySelector('#confirm').value === value &&
          document.querySelector('#current').value === 'owned-current-fixture',
    ...
    if (workerRestartLifecycleMode) {
      ...
      assert(
        oldOffer && oldOffer.expiresAt >= Date.now() + 10_000,
        'worker_restart_old_offer_missing_or_near_expiry',
      );
      ...
      try {
        ...
        const oldTarget = targetSnapshot.targetInfos.find(
        ...
        if (!oldTarget || typeof refreshWorker !== 'function') {
          ...
          for (let attempt = 0; attempt < 20; attempt += 1) {
            oldVersion = versions.find(
              (version) =>
                ...
                version.targetId === oldTarget.targetId &&
          ...
          if (!oldVersion) {
            ...
            lifecycle.workerRealmMarkerPresentBeforeStop = await previousWorker.evaluate(
              (property) => {
            ...
            assert(
              realmMarkerSet && typeof workerRuntimeId === 'string' && workerRuntimeId.length > 0,
              'worker_restart_realm_marker_not_set',
            );
            ...
            lifecycle.uiCandidateBeforeTtl =
              uiCandidate.expiresAt > lifecycle.uiCandidateObservedAt + 10_000;
            assert(
              lifecycle.uiCandidatePreStop.regionPresent &&
                lifecycle.uiCandidatePreStop.generatedCandidatePresent &&
                lifecycle.uiCandidatePreStop.offerPresent &&
                lifecycle.uiCandidateBeforeTtl &&
                lifecycle.oldOfferRemainingMs >= 10_000,
              'worker_restart_ui_candidate_not_live_before_stop',
            );
            await cdp.send('ServiceWorker.stopWorker', { versionId: oldVersion.versionId });
            let oldTargetGone = false;
            for (let attempt = 0; attempt < 30; attempt += 1) {
              const targets = await cdp.send('Target.getTargets');
              if (!targets.targetInfos.some((target) => target.targetId === oldTarget.targetId)) {
                oldTargetGone = true;
                break;
              }
              await wait(100);
            ...
            if (!oldTargetGone) {
              ...
              await panel.waitFor(
                `!!(${lifecycleSection}) && !(${lifecycleSection})?.querySelector('code') && !(${lifecycleSection})?.querySelector('[name="generated-password-target"]') && !!(${lifecycleButton('Generate')}) && !(${lifecycleButton('Generate')}).disabled && (${lifecycleSection})?.innerText.includes('generator connection changed')`,
              );
              lifecycle.uiCandidateClearedOnDisconnect = true;
              lifecycle.uiCandidateClearedAt = Date.now();
              assert(
                lifecycle.uiCandidateClearedAt < uiCandidate.expiresAt,
                'worker_restart_ui_clear_after_expiry',
              );
              ...
              lifecycle.samePanelTargetAndDocument =
                afterPanel.targetId === uiCandidate.identity.targetId &&
                afterPanel.documentId === uiCandidate.identity.documentId;
              ...
              const realm = await worker.evaluate(
                ({ property, runtimeId, url }) => ({
              ...
              lifecycle.workerRealmReset =
                realm?.markerAbsent === true &&
                realm?.exactRuntime === true &&
                realm?.exactWorkerUrl === true;
              assert(lifecycle.workerRealmReset, 'worker_restart_realm_not_reset');
              lifecycle.oldOfferRemainingMs = oldOffer.expiresAt - Date.now();
              assert(
                lifecycle.oldOfferRemainingMs >= 10_000,
                'worker_restart_old_offer_insufficient_ttl_headroom',
              );
              ...
              assert(
                lifecycle.oldOfferCompletedBeforeExpiry,
                'worker_restart_old_offer_refusal_after_expiry',
              );
              lifecycle.oldOfferFieldsUnchanged = await unchanged(page);
              assert(
                lifecycle.oldOfferStatus === 'stale' && lifecycle.oldOfferFieldsUnchanged,
                'worker_restart_old_offer_survived',
              );
        ...
        if (realmMarkerSet) {
          try {
            await previousWorker.evaluate((property) => {
    ...
    if (windowSwitchLifecycleMode) {
      ...
      assert(
        oldOffer && oldOffer.expiresAt >= Date.now() + 10_000,
        'window_switch_old_offer_missing_or_near_expiry',
      );
      ...
      lifecycle.uiCandidateBeforeTtl =
        uiCandidate.expiresAt > lifecycle.uiCandidateObservedAt + 10_000;
      assert(
        lifecycle.uiCandidatePreSwitch.regionPresent &&
          lifecycle.uiCandidatePreSwitch.generatedCandidatePresent &&
          lifecycle.uiCandidatePreSwitch.offerPresent &&
          lifecycle.uiCandidateBeforeTtl,
        'window_switch_ui_candidate_missing',
      );
      ...
      assert(
        before.focused && before.focusedWindowId === before.windowId && before.tabActive,
        'window_switch_fixture_not_focused_before_transition',
      );
      ...
      const closeOwnedExtraWindow = async () => {
        ...
        if (Number.isInteger(extraWindowId)) {
          ...
          for (let attempt = 0; attempt < 20; attempt += 1) {
            const windows = await worker.evaluate(async () =>
            ...
            cleanupVerified =
              removed === true &&
              !windows.includes(extraWindowId) &&
              windows.length === baselineWindowIds.length &&
              baselineWindowIds.every((id) => windows.includes(id)) &&
              Array.isArray(targets?.targetInfos) &&
              !targets.targetInfos.some((target) => target.targetId === createdTargetId);
            if (cleanupVerified) break;
            await wait(100);
          }
          lifecycle.ownedExtraWindowCleanup = cleanupVerified
            ? 'verified_chrome_window_removed'
            : 'failed_chrome_window_removal_verification';
        } else if (createdTargetId) {
          const closed = await cdp
            .send('Target.closeTarget', { targetId: createdTargetId })
            .catch(() => ({ success: false }));
          for (let attempt = 0; attempt < 20; attempt += 1) {
            const windows = await worker.evaluate(async () =>
              (await chrome.windows.getAll({ windowTypes: ['normal'] }))
          ...
          lifecycle.ownedExtraWindowCleanup = cleanupVerified
            ? 'verified_created_target_closed'
            : 'failed_created_target_cleanup_verification';
        } else {
      ...
      try {
        baselineWindowIds = await worker.evaluate(async () =>
          (await chrome.windows.getAll({ windowTypes: ['normal'] }))
            .map((window) => window.id)
            .filter(Number.isInteger)
            .sort((left, right) => left - right),
        ...
        for (let attempt = 0; attempt < 20; attempt += 1) {
          const delta = await worker.evaluate(
            async (baseline) =>
              (await chrome.windows.getAll({ windowTypes: ['normal'] }))
                .map((window) => window.id)
                .filter((id) => Number.isInteger(id) && !baseline.includes(id)),
            baselineWindowIds,
          );
        ...
        if (!Number.isInteger(extraWindowId)) {
          ...
          lifecycle.transition = await worker.evaluate(
            async ({ originalWindowId, otherWindowId, expectedTabId }) => {
            ...
            {
              originalWindowId: before.windowId,
              otherWindowId: extraWindowId,
              expectedTabId: tabId,
            },
          );
          ...
          assert(
            lifecycle.transition.awayFocusedOther && lifecycle.transition.originalTabActive,
            'window_switch_away_focus_transition_unproven',
          );
          const windowSwitchUiState = () =>
            panel.evaluate(`(() => {
            const root = ${lifecycleSection};
          ...
          try {
            ...
            lifecycle.uiClearingDisposition = clearedState.sectionPresent
              ? 'present_cleared'
              : 'absent';
            lifecycle.uiWaitOutcome = 'cleared';
          } catch (error) {
            lifecycle.uiWaitOutcome = error instanceof Error ? error.message : 'unknown_error';
            lifecycle.uiAfterClearWait = await windowSwitchUiState().catch(() => ({
              unavailable: true,
            }));
            const panelAfterFailedWait = await panelIdentity().catch(() => null);
            lifecycle.samePanelTargetAndDocumentAfterFailedWait =
              panelAfterFailedWait !== null &&
            ...
            lifecycle.focusEvent = await worker
              .evaluate(
                ({ key, expectedWindowId }) => {
                  const state = globalThis[key]?.state;
                  return state
                    ? {
                        eventCount: state.eventCount,
                        focusedOtherObserved: state.observedWindowIds.has(expectedWindowId),
                      }
                    : { unavailable: true };
                },
            ...
            lifecycle.panelLifecycleSignal = await panel
              .evaluate(`globalThis.__vaultCanaryWindowSwitchProbe?.state ?? { unavailable: true }`)
              .catch(() => ({ unavailable: true }));
          }
          ...
          assert(
            lifecycle.uiCandidateClearedAt < uiCandidate.expiresAt,
            'window_switch_ui_clear_after_expiry',
          );
          const afterPanel = await panelIdentity();
          lifecycle.samePanelTargetAndDocument =
            afterPanel.targetId === uiCandidate.identity.targetId &&
          ...
          assert(
            lifecycle.oldOfferRemainingMs >= 10_000,
            'window_switch_old_offer_insufficient_ttl_headroom',
          );
          ...
          assert(
            lifecycle.otherWindowFocusedBeforeOldUse,
            'window_switch_other_window_not_focused_before_old_use',
          );
          const oldUse = await use(oldOffer.id);
          lifecycle.oldOfferStatus = oldUse?.status ?? 'missing_response';
          lifecycle.oldOfferCompletedBeforeExpiry = Date.now() < oldOffer.expiresAt;
          assert(
            lifecycle.oldOfferCompletedBeforeExpiry,
            'window_switch_old_offer_refusal_after_expiry',
          );
          ...
          assert(
            lifecycle.otherWindowFocusedThroughOldUse,
            'window_switch_other_window_lost_focus_during_old_use',
          );
          lifecycle.oldOfferFieldsUnchanged = await unchanged(page);
          assert(
            lifecycle.oldOfferStatus === 'stale' && lifecycle.oldOfferFieldsUnchanged,
            'window_switch_old_offer_survived',
          );
          assert(
            await closeOwnedExtraWindow(),
            'window_switch_owned_extra_window_cleanup_unverified',
          );
          lifecycle.originalFocusedAfterClose = await worker.evaluate(
            async ({ windowId, expectedTabId }) => {
          ...
          assert(
            lifecycle.originalFocusedAfterClose,
            'window_switch_original_focus_not_restored_after_close',
          );
        ...
        if (!extraWindowClosed && (Number.isInteger(extraWindowId) || createdTargetId))
          await closeOwnedExtraWindow();
        if (!extraWindowClosed && lifecycle.disposition === 'passed')
          lifecycle.disposition = 'failed_owned_extra_window_cleanup_unverified';
      }
      if (lifecycle.disposition !== 'passed') {
    ...
    if (panelCloseLifecycleMode) {
      const lifecycle = (evidence.panelCloseLifecycle = {
        disposition: 'in_progress',
        offerKind: 'raw_discover_offer',
      });
      ...
      assert(
        oldOffer && oldOffer.expiresAt > Date.now() + 1000,
        'panel_close_old_offer_missing_or_near_expiry',
      );
      ...
      assert(
        before.tabActive && before.focused && before.windowId === before.focusedWindowId,
        'panel_close_fixture_not_active_before_transition',
      );
      await worker.evaluate((id) => {
      ...
      const cdp = await context.newCDPSession(page);
      try {
        const supportsClose = await worker.evaluate(
          () => typeof chrome.sidePanel?.close === 'function',
        );
        if (!supportsClose || typeof reopenPanelFromAction !== 'function') {
          lifecycle.disposition = !supportsClose
            ? 'not_tested_side_panel_close_unavailable'
            : 'not_tested_action_popup_reopen_unavailable';
        } else {
          const oldTargetId = panel.targetId;
          const beforeContexts = await worker.evaluate(() =>
            chrome.runtime.getContexts({ contextTypes: ['SIDE_PANEL'] }),
          );
          ...
          if (closeResult !== 'requested') {
            ...
            for (let attempt = 0; attempt < 30; attempt += 1) {
              ...
              if (
                contexts.length === 0 &&
                !targets.targetInfos.some((target) => target.targetId === oldTargetId)
              ) {
                closed = true;
                break;
              }
            ...
            if (!closed) {
              ...
              if (!reopened.opened) {
                ...
                const afterContexts = await worker.evaluate(() =>
                  chrome.runtime.getContexts({ contextTypes: ['SIDE_PANEL'] }),
                );
                assert(
                  afterContexts.length === 1 &&
                    afterContexts[0].documentId !== beforeContexts[0].documentId &&
                    panel.targetId !== oldTargetId,
                  'panel_close_reopen_not_new_document',
                );
                ...
                assert(
                  lifecycle.reopenedConnectionDistinct,
                  'panel_close_reopened_connection_reused',
                );
                checkpoint('generator_panel_close_old_offer');
                lifecycle.oldOfferRemainingMs = oldOffer.expiresAt - Date.now();
                assert(
                  lifecycle.oldOfferRemainingMs >= 10_000,
                  'panel_close_old_offer_insufficient_ttl_headroom',
                );
                const oldUse = await use(oldOffer.id);
                lifecycle.oldOfferStatus = oldUse?.status ?? 'missing_response';
                lifecycle.oldOfferCompletedBeforeExpiry = Date.now() < oldOffer.expiresAt;
                assert(
                  lifecycle.oldOfferCompletedBeforeExpiry,
                  'panel_close_old_offer_refusal_after_expiry',
                );
                lifecycle.oldOfferFieldsUnchanged = await unchanged(page);
                const after = await worker.evaluate(async (id) => {
                  ...
                  return {
                    ...
                    focusedWindowId: focused.id,
                    focused: focused.focused === true,
                    activated: tracker?.state?.activated ?? -1,
                    focusChanged: tracker?.state?.focusChanged ?? -1,
                  };
                }, tabId);
                ...
                assert(
                  after.tabId === before.tabId &&
                    ...
                    after.tabActive &&
                    after.focused &&
                    after.focusedWindowId === before.focusedWindowId &&
                    after.activated === 0 &&
                    after.focusChanged === 0,
                  'panel_close_transition_changed_tab_or_window',
                );
                ...
                assert(
                  (await use(freshOffer.id))?.status === 'filled',
                  'panel_close_fresh_offer_refused',
                );
    ...
    const button = (label) =>
      `Array.from((${section}).querySelectorAll("button")).find((button) => button.textContent.trim() === ${JSON.stringify(label)})`;
    ...
    const captureGenerateTimeout = async (kind, phase, knobResolve) => {
      ...
      evidence.uiTimeoutDiagnostic = {
        phase,
        kind,
        ...panelState,
        knobResolve: knobResolve.snapshot(),
      };
    ...
    replaceOwnedCopiedClipboard = async (expectedValue) => {
      ...
      try {
        ...
        const nativePasteMatches = async (expected) => {
          ...
          do {
            await page.evaluate(() => {
              document.querySelector('#clipboard-fixture').value = '';
            });
    ...
    if (
      await panel.evaluate(
        `(${button('Password generator')})?.getAttribute('aria-expanded') !== 'true'`,
      )
    )
    ...
    for (const kind of ['Password', 'Passphrase']) {
      ...
      await wait(1000);
      evidence.uiAfterGenerate = await panel.evaluate(
        `({ generatorPresent: !!(${section}), selectedVault: document.querySelector('[title="Vault"]')?.getAttribute('aria-selected'), generatedCodePresent: !!(${section})?.querySelector('code'), signInPresent: /Sign in/.test(document.body.innerText), loadingPresent: !!document.querySelector('.animate-spin') })`,
      );
      checkpoint(phase);
      await waitForGeneratedValue(kind, phase, knobResolve);
      assert(
        await panel.evaluate(
          `(${section}).querySelector('code').textContent === '••••••••••••••••••••••••'`,
        ),
        'generator_value_not_masked',
      );
      ...
      assert(
        typeof synthetic === 'string' &&
          (kind === 'Password'
            ? synthetic.length === 24
            : synthetic.length >= 11 && synthetic.includes('-')),
        'generator_default_value_invalid',
      );
      if (kind === 'Password' && copyEnabled) {
        ...
        await panel.waitFor(
          `(${section})?.innerText.includes('Copied. Clipboard is not cleared automatically.')`,
        );
        await page.bringToFront();
        ...
        assert(
          clipboardCustody.disposition === 'owned_value_replaced',
          'generator_copy_native_paste_or_owned_replacement_mismatch',
        );
      ...
      await panel.waitFor(
        `!!(${section})?.querySelector('[aria-label="Reveal generated value"]') && (${section})?.innerText.includes('Generated.')`,
      );
      await panel.click(`(${section}).querySelector('[aria-label="Reveal generated value"]')`);
      ...
      await panel.waitFor(
        `!(${section}).querySelector('code') && /Filled/.test((${section}).innerText)`,
      );
    ...
    await dispatchAndWaitForKeyboard(
      'escape_generated_clear',
      { key: 'Escape', code: 'Escape', windowsVirtualKeyCode: 27 },
      `(${section})?.querySelector('[aria-expanded="false"]') && !(${section})?.querySelector('code') && document.activeElement === (${button('Password generator')})`,
    );
    ...
    await dispatchAndWaitForKeyboard(
      ...
      `(${section})?.querySelector('[aria-expanded="true"]') && !(${section})?.querySelector('code')`,
    );
    await dispatchAndWaitForKeyboard(
      'escape_after_enter',
      { key: 'Escape', code: 'Escape', windowsVirtualKeyCode: 27 },
      `(${section})?.querySelector('[aria-expanded="false"]') && document.activeElement === (${button('Password generator')})`,
    );
    await dispatchAndWaitForKeyboard(
      'space_opens',
    ...
    await dispatchAndWaitForKeyboard(
      ...
      `document.activeElement === (${button('Password')}) && document.activeElement.matches(':focus-visible')`,
    );
    ...
    evidence.remaining = [
      ...
      ...(!windowSwitchLifecycleMode
        ? ['normal-window switch needs a separate real-browser case']
        : []),
      ...(windowSwitchLifecycleMode
        ? ['two-open-normal-window return focus remains untested in the headless runtime']
        : []),
    ...
    if (
      copyEnabled &&
      clipboardCustody.copyAttempted &&
      clipboardCustody.disposition !== 'owned_value_replaced'
    ) {
      try {
        clipboardCustody.disposition = await replaceOwnedCopiedClipboard(copiedSyntheticValue);
    ...
    evidence.ownedFixturePageClosed =
      cleanup[0].status === 'fulfilled' && (!page || page.isClosed());
    evidence.ownedFixtureServersClosed =
      !!top && !!child && cleanup[1].status === 'fulfilled' && cleanup[2].status === 'fulfilled';
