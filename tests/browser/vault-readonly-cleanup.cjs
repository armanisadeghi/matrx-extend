...
const bool = (value, key, expected = true) =>
  object(value) && own(value, key) && value[key] === expected;
const integer = (value, key, expected) =>
  object(value) && own(value, key) && Number.isInteger(value[key]) && value[key] === expected;
const positive = (value, key) =>
  object(value) && own(value, key) && Number.isInteger(value[key]) && value[key] > 0;
...
function hasObservedReadOnlyCleanup(proof) {
  if (
    ...
    !Array.isArray(proof.ownedFixtureIds) ||
    proof.ownedFixtureIds.length !== 0 ||
    ...
    (own(proof, 'generator') &&
      (!object(proof.generator) || !bool(proof.generator, 'ownedFixtureServersClosed'))) ||
...
function hasPreBaselineAuthenticatedCleanup(proof) {
  return (
    ...
    bool(proof.authStorage, 'profilePresent') &&
    bool(proof.authStorage, 'accessTokenPresent') &&
    !own(proof, 'baselineMetadataSha256') &&
    !own(proof.admission, 'baselineRead') &&
    integer(proof.admission, 'fixtureWrites', 0) &&
    bool(proof.admission, 'noFixtureWrites') &&
    integer(proof, 'vaultMutationRequests', 0) &&
    zeroItemPosts(proof.vaultItemPosts) &&
    Array.isArray(proof.ownedCreateMutationKeys) &&
    ...
    !own(proof.cleanup, 'finalBaselineIdSetMatches') &&
    !own(proof.cleanup, 'finalBaselineMetadataMatches') &&
    proof.cleanup.localFixtureServerClosed === 'not_started' &&
    integer(proof.cleanup, 'localAuthLogoutStatus', 204) &&
...
const PRE_AUTH_OAUTH_STATES = {
  oauth_popup_navigation: {
    failureCategory: 'oauth_popup_navigation_failed',
    popupNavigated: false,
    popupSignInClicked: false,
  },
  oauth_popup_sign_in_click: {
    failureCategory: 'oauth_popup_sign_in_click_failed',
    popupNavigated: true,
    popupSignInClicked: false,
  },
...
function hasPreAuthNoWriteCleanup(proof) {
  ...
  const identityAbsent =
    object(proof) &&
    object(proof.checks) &&
    (!own(proof.checks, 'independentAdminIdentity') ||
      proof.checks.independentAdminIdentity === false);
  return (
    object(proof) &&
    proof.schema === 3 &&
    ...
    !own(proof, 'authStorage') &&
    !own(proof, 'authTransport') &&
    !own(proof, 'identityProof') &&
    identityAbsent &&
    admissionClear &&
    !own(proof, 'baselineMetadataSha256') &&
    !own(proof, 'baselineItems') &&
    !own(proof, 'authenticator') &&
    !own(proof, 'authenticatorJournal') &&
    !own(proof.checks, 'enrolledAuthenticatorPreserved') &&
    !own(proof.cleanup, 'authenticator') &&
    ...
    bool(proof.cleanup, 'profileRemoved') &&
    proof.cleanup.localCredentialDisposal === 'profile_removed' &&
    !own(proof.cleanup, 'localAuthLogoutStatus') &&
    !own(proof.cleanup, 'remoteAuthRevocationStatus') &&
    bool(proof.networkJournal, 'ownerVerified') &&
    proof.networkJournal?.journalSemanticVersion === 2 &&
