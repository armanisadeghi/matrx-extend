...
const chrome = '/Users/armanisadeghi/Library/Caches/ms-playwright/chromium-1243/chrome-mac-arm64/Google Chrome for Testing.app/Contents/MacOS/Google Chrome for Testing';
...
function attachTarget(rawCdp, targetId) {
  return rawCdp.send('Target.attachToTarget', { targetId, flatten: true }).then(({ sessionId }) => ({
  ...
  return {
    ...
    setup(buildApi) {
      ...
      buildApi.onLoad({ filter: /.*/, namespace: 'diagnostic-stub' }, (args) => ({
        contents: stubs[args.path].replaceAll('ORIGIN', `http://127.0.0.1:${port}`), loader: 'ts',
      }));
      buildApi.onLoad({ filter: /inline-suggestions-host\.ts$/ }, async (args) => {
        ...
        source = trace + source;
        source = source.replaceAll("}).catch(() => null);", "}).catch(() => { __usernameFirstDiagTrace('scripting_rejected'); return null; });");
      ...
      buildApi.onLoad({ filter: /fill-primitive\.ts$/ }, async (args) => {
        ...
        source = source.replace("      if (`${location.origin}${location.pathname}` !== group.pageUrl) return false;", "      if (`${location.origin}${location.pathname}` !== group.pageUrl) { __diag('safe_page'); return false; }");
        source = source.replace('      if (!sameNode(group.anchor, currentAnchor)) return false;', "      if (!sameNode(group.anchor, currentAnchor)) { __diag('safe_anchor'); return false; }");
        source = source.replace('      if (!sameNode(group.username, originals.username)) return false;', "      if (!sameNode(group.username, originals.username)) { __diag('safe_username'); return false; }");
        source = source.replace('      if (!sameNode(group.password, originals.password)) return false;', "      __diag(group.password === null ? 'password_null' : group.password === undefined ? 'password_undefined' : 'password_present');\n      if (!sameNode(group.password, originals.password)) { __diag('safe_password'); return false; }");
        source = source.replace("      if (requirePanelFocus && document.visibilityState !== 'visible') return false;", "      if (requirePanelFocus && document.visibilityState !== 'visible') { __diag('safe_visibility'); return false; }");
        source = source.replace("        return false;\n      const form = currentAnchor.form;", "        { __diag('safe_active'); return false; }\n      const form = currentAnchor.form;");
        source = source.replace("      if (classify(form, null).kind === 'unsafe') return false;\n      return true;", "      if (classify(form, null).kind === 'unsafe') { __diag('safe_form'); return false; }\n      __diag('safe_ok');\n      return true;");
        return { contents: source, loader: 'ts' };
      });
    },
  };
...
let run = { passed: false, fieldKeys: [], trace: [], websiteFocus: null, websiteFocusAfter: null, passwordPresent: null, profileOwnerVerified: false };
let cleanupFailed = false;
try {
  server = http.createServer((_request, response) => response.end(renderSavedFormMatrixHTML('username_first')));
  ...
  const bg = await build({ stdin: { contents: `import {registerInlineCredentialSuggestionHost} from '@/lib/credentials/inline-suggestions-host';const originalExecute=chrome.scripting.executeScript.bind(chrome.scripting);chrome.scripting.executeScript=async(request)=>{const op=request?.args?.[0]?.operation??'unknown';try{const response=await originalExecute(request);const result=response?.[0]?.result;const outcome=op==='fill'?(result?.ok===true?'filled':result?.reason==='partial_manual_check'?'partial':'refused'):'ok';const prior=(await chrome.storage.local.get('__usernameFirstDiagScripting')).__usernameFirstDiagScripting??[];void chrome.storage.local.set({__usernameFirstDiagScripting:[...prior,op+':'+outcome]});return response}catch(error){const prior=(await chrome.storage.local.get('__usernameFirstDiagScripting')).__usernameFirstDiagScripting??[];void chrome.storage.local.set({__usernameFirstDiagScripting:[...prior,op+':rejected']});throw error}};registerInlineCredentialSuggestionHost()`, resolveDir: root, loader: 'ts' }, bundle: true, format: 'iife', write: false, plugins: [plugin] });
  const content = await build({ stdin: { contents: `import {mountGenerationTargetRegistry} from '@/lib/credentials/generation-targets';import {mountInlineCredentialSuggestions} from '@/lib/credentials/inline-suggestions';mountGenerationTargetRegistry();mountInlineCredentialSuggestions()`, resolveDir: root, loader: 'ts' }, bundle: true, format: 'iife', write: false, plugins: [plugin] });
  await Promise.all([
    writeFile(join(dir, 'background.js'), bg.outputFiles[0].text),
    writeFile(join(dir, 'content.js'), content.outputFiles[0].text),
    writeFile(join(dir, 'sidepanel.html'), '<button id="fill">Fill</button><script src="sidepanel.js"></script>'),
    writeFile(join(dir, 'sidepanel.js'), `document.querySelector('#fill').onclick=async()=>{const{tabId}=await chrome.storage.local.get('tabId');const offer=await chrome.runtime.sendMessage({__matrx:true,kind:'credential-suggestions:panel-status',payload:{tabId}});const result=await chrome.runtime.sendMessage({__matrx:true,kind:'credential-suggestions:panel-fill',payload:{tabId,offerId:offer.offerId,itemId:'${itemId}'}});document.body.dataset.result=JSON.stringify(result)}`),
    writeFile(join(dir, 'popup.html'), '<button id="open">Open panel</button><script src="popup.js"></script>'),
    writeFile(join(dir, 'popup.js'), "document.querySelector('#open').onclick=async()=>{try{const windowId=(await chrome.windows.getCurrent()).id;await chrome.sidePanel.open({windowId});document.body.dataset.opened='true'}catch{document.body.dataset.opened='refused'}}"),
    writeFile(join(dir, 'manifest.json'), JSON.stringify({ manifest_version: 3, name: 'username-first-native-diagnostic', version: '1.0', permissions: ['scripting', 'tabs', 'webNavigation', 'storage', 'sidePanel'], host_permissions: ['http://127.0.0.1/*'], background: { service_worker: 'background.js' }, action: { default_popup: 'popup.html' }, content_scripts: [{ matches: ['http://127.0.0.1/*'], js: ['content.js'], run_at: 'document_start' }], side_panel: { default_path: 'sidepanel.html' } })),
  ]);
  await mkdir(profile, { recursive: true, mode: 0o700 });
  const preparedProfile = await prepareOwnedProfile(profile);
  context = await chromium.launchPersistentContext(profile, { executablePath: chrome, headless: true, args: ['--headless=new', '--remote-debugging-address=127.0.0.1', '--remote-debugging-port=0', `--disable-extensions-except=${dir}`, `--load-extension=${dir}`] });
  ...
  for (let attempt = 0; attempt < 30; attempt += 1) {
    const ready = await worker.evaluate(async (id) => (await chrome.scripting.executeScript({ target: { tabId: id }, func: () => Boolean(globalThis.__matrx_generation_target_registry__) }))[0]?.result === true, tabId);
    if (ready) break;
  ...
  const workerTarget = targets.targetInfos.find((target) => target.type === 'service_worker' && target.url.endsWith('/background.js'));
  if (!workerTarget) throw new Error('extension_worker_target_missing');
  const windowId = await worker.evaluate((id) => chrome.tabs.get(id).then((tab) => tab.windowId), tabId);
  const popupUrl = `chrome-extension://${extensionId}/popup.html`;
  const beforePopup = await rawCdp.send('Target.getTargets');
  const knownPopupIds = new Set(beforePopup.targetInfos.filter((target) => target.url === popupUrl).map((target) => target.targetId));
  const popupRequested = await worker.evaluate(async (id) => {
    try { await chrome.action.openPopup({ windowId: id }); return true; } catch { return false; }
  }, windowId);
  if (!popupRequested) throw new Error('native_action_popup_refused');
  let popupTarget;
  for (let attempt = 0; attempt < 30; attempt += 1) {
    const liveTargets = await rawCdp.send('Target.getTargets');
    popupTarget = liveTargets.targetInfos.find((target) => target.type === 'page' && target.url === popupUrl && !knownPopupIds.has(target.targetId));
  ...
  const popupBox = await popup.send('Runtime.evaluate', { expression: "(() => { const element=document.querySelector('#open'); if(!element)return null; const rect=element.getBoundingClientRect(); return {x:rect.x+rect.width/2,y:rect.y+rect.height/2,width:rect.width,height:rect.height} })()", returnByValue: true });
  const box = popupBox.result.value;
  if (!(box?.width > 0 && box?.height > 0)) throw new Error('native_action_popup_control_missing');
  const openedFromGesture = await popup.send('Runtime.evaluate', { expression: "document.querySelector('#open')?.click()", userGesture: true, awaitPromise: true, returnByValue: true });
  if (openedFromGesture.exceptionDetails) throw new Error('native_side_panel_popup_click_refused');
  let popupOpened;
  for (let attempt = 0; attempt < 20; attempt += 1) {
    const state = await popup.send('Runtime.evaluate', { expression: 'document.body.dataset.opened || null', returnByValue: true });
    popupOpened = state.result.value;
    if (popupOpened) break;
    await sleep(100);
  }
  if (popupOpened !== 'true') throw new Error(`native_side_panel_popup_${popupOpened ?? 'timeout'}`);
  await popup.detach();
  let panelTarget;
  for (let attempt = 0; attempt < 40; attempt += 1) {
    const contexts = await worker.evaluate(() => chrome.runtime.getContexts({ contextTypes: ['SIDE_PANEL'] }));
    const liveTargets = await rawCdp.send('Target.getTargets');
    panelTarget = liveTargets.targetInfos.find((target) => target.type === 'page' && target.url === `chrome-extension://${extensionId}/sidepanel.html`);
    if (contexts.some((entry) => entry.documentUrl === `chrome-extension://${extensionId}/sidepanel.html` && entry.tabId === -1) && panelTarget) break;
    await sleep(250);
  }
  ...
  const websiteFocus = await worker.evaluate(async (id) => (await chrome.scripting.executeScript({ target: { tabId: id }, func: () => ({ activeUsername: document.activeElement?.id === 'username', documentFocused: document.hasFocus(), visible: document.visibilityState === 'visible' }) }))[0]?.result, tabId);
  const panelButton = await panel.send('Runtime.evaluate', { expression: "(() => { const element=document.querySelector('#fill'); if(!element)return null; element.scrollIntoView({block:'nearest',inline:'nearest'}); const rect=element.getBoundingClientRect(); const hit=document.elementFromPoint(rect.x+rect.width/2,rect.y+rect.height/2); return {x:rect.x+rect.width/2,y:rect.y+rect.height/2,width:rect.width,height:rect.height,hit:!!hit&&(hit===element||element.contains(hit))} })()", returnByValue: true });
  const button = panelButton.result.value;
  if (!(button?.width > 0 && button?.height > 0 && button.hit)) throw new Error('native_side_panel_fill_control_missing');
  await panel.send('Input.dispatchMouseEvent', { type: 'mouseMoved', x: button.x, y: button.y });
  await panel.send('Input.dispatchMouseEvent', { type: 'mousePressed', x: button.x, y: button.y, button: 'left', clickCount: 1 });
  await panel.send('Input.dispatchMouseEvent', { type: 'mouseReleased', x: button.x, y: button.y, button: 'left', clickCount: 1 });
  let panelResult;
  for (let attempt = 0; attempt < 30; attempt += 1) {
    panelResult = await panel.send('Runtime.evaluate', { expression: 'document.body.dataset.result || null', returnByValue: true });
    if (panelResult.result.value) break;
    await sleep(250);
  }
  const workerSession = await attachTarget(rawCdp, workerTarget.targetId);
  const diagnostic = await workerSession.send('Runtime.evaluate', { expression: 'globalThis.__usernameFirstDiagTrace || []', returnByValue: true });
  const storage = await worker.evaluate(() => chrome.storage.local.get(['diagFieldKeys', '__usernameFirstDiagTrace', '__usernameFirstDiagScripting']));
  const primitiveTrace = await worker.evaluate(async (id) => (await chrome.scripting.executeScript({ target: { tabId: id }, func: () => globalThis.__usernameFirstPrimitiveTrace__ ?? globalThis.__usernameFirstPrimitiveTrace ?? [] }))[0]?.result, tabId);
  const passwordPresent = await page.locator('#password').count() === 1;
  const websiteFocusAfter = await worker.evaluate(async (id) => (await chrome.scripting.executeScript({ target: { tabId: id }, func: () => ({ activeUsername: document.activeElement?.id === 'username', documentFocused: document.hasFocus(), visible: document.visibilityState === 'visible' }) }))[0]?.result, tabId);
  const value = await page.locator('#username').inputValue();
  const result = panelResult?.result?.value ?? null;
  run = { passed: String(result).includes('filled') && value === 'fixture-user', fieldKeys: storage.diagFieldKeys ?? [], trace: diagnostic.result.value ?? [], websiteFocus, websiteFocusAfter, passwordPresent, profileOwnerVerified: rawCdp.ownerVerified === true };
  ...
  const proof = {
    ok: run.passed && !cleanupFailed && cleanup.extensionRemoved && cleanup.profileRemoved && cleanup.serverClosed,
    scope: 'native-extension-sidepanel-diagnostic',
    passingAfter: run,
    failingBefore: { panelStatus: 'stale', primitiveTrace: ['password_undefined', 'safe_password'], writer: 'refused' },
    cleanup: { browserClosed: !context?.browser()?.isConnected(), cdpDetached: !cleanupFailed, profileGone: cleanup.profileRemoved, extensionGone: cleanup.extensionRemoved, serverGone: cleanup.serverClosed },
    sourceHashes: hashes,
  };
  await writeFile(proofPath, `${JSON.stringify(proof)}\n`, { mode: 0o600 });
  if (!proof.ok) { console.error(JSON.stringify({ ok: false, proofPath })); cleanupFailed = true; }
}
