...
async function setEnginePort(panel) {
  ...
  await waitFor(
    'port_input_typed',
    () => portControlState(panel),
    (state) => state?.value === String(PORT) && state.focused,
  );
  await click(panel, 'button', 'Set');
  await waitFor(
    'port_override_persisted',
    async () => ({
      control: await portControlState(panel),
      storage: await storageState(panel),
    }),
...
async function seedDisposableSession(panel) {
  ...
  await waitFor(
    'session_fixture_persisted',
    () => storageState(panel),
    (s) => s.sessionFixtureMatches,
  );
}
...
try {
  ...
  assert.equal(
    receipt.version,
    packageJson.version,
    'released build must match current package version',
  );
  ...
  const result = await runNativeSidepanelQa({
    exercisePanel: async ({ panel }) => {
      ...
      report.steps.push({
        action: 'Set local engine port and disposable guest session fixture',
        result: 'independent local and session values persisted in isolated profile',
      });
      ...
      assert.equal(
        beforeConfirm.hasSessionFixture,
        true,
        'Cancel must preserve guest session fixture',
      );
      assert.equal(
        beforeConfirm.sessionFixtureMatches,
        true,
        'Cancel must preserve exact guest session value',
      );
      report.steps.push({
        action: 'Cancel Clear local data',
        result: 'dialog closed; Dark theme, port override, and session fixture preserved',
      });

      await click(panel, 'button', 'Clear local data on this device');
      await waitFor(
        'reset_dialog_reopened',
        () => panelState(panel),
        (s) => s.dialog,
      );
      await click(panel, 'dialog', 'Clear & sign out');
      await waitFor(
        'local_and_session_cleared',
        () => storageState(panel),
        (s) => !s.hasSettingsKey && s.port === null && !s.hasSessionFixture,
      );
      ...
      assert.deepEqual(
        beforeConfirm.localKeys.filter((key) => after.localKeys.includes(key)),
        [],
        'all pre-confirm local keys must be cleared',
      );
      assert.deepEqual(
        beforeConfirm.sessionKeys.filter((key) => after.sessionKeys.includes(key)),
        [],
        'all pre-confirm session keys must be cleared',
      );
      report.steps.push({
        action: 'Confirm Clear & sign out',
        result: 'all prior local and session keys removed',
      });
      ...
      report.steps.push({
        action: 'Reload native panel',
        result:
          'guest controls and System default visible; theme, port, and session fixture absent',
      });
    },
  });
