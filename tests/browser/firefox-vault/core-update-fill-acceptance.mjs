...
async function elementId(driver, selector) {
  const value = await driver.wdPost(driver.base, `/session/${driver.sessionId}/element`, {
    using: 'css selector',
    value: selector,
  });
    ...
    (document, expectedName) => {
      ...
      for (let depth = 0; card && depth < 4; depth += 1, card = card.parentElement) {
        ...
        const matching = buttons.filter(
          (button) => button.textContent?.replace(/^Update\s*/, '').trim() === expectedName,
        );
...
async function exactFillSelector(adapter, targetName) {
  return adapter.waitFor(
    (document, expectedName) => {
      const cards = [...document.querySelectorAll('li')].filter(
        (card) => card.querySelector('span')?.textContent?.trim() === expectedName,
      );
      if (cards.length !== 1) return null;
      const buttons = [...cards[0].querySelectorAll('button')].filter(
...
async function valuesMatch({ base, sessionId, wdPost }, username, password) {
  const value = await wdPost(base, `/session/${sessionId}/execute/sync`, {
    script:
      'return { usernameMatches: document.querySelector("#username")?.value === arguments[0], passwordMatches: document.querySelector("#password")?.value === arguments[1] };',
...
export async function runFirefoxCoreUpdateFillChecks({
  ...
  assert.ok(
    ...
    'core_update_fill_fixture_contract_invalid',
  );
  ...
  let primary;
  let cleanup;
  proof.coreUpdateFill = {
    ok: false,
    updateSubmittedOnce: false,
    exactUpdateChoiceVisible: false,
    trustedUpdateClicked: false,
    pendingRemoved: false,
    updatedValuesVerified: false,
    freshFillFocused: false,
    exactFillVisible: false,
    trustedFillClicked: false,
    filledExactValues: false,
    noExtraSubmission: false,
    updateTabClosed: false,
    fillTabClosed: false,
    originalWindowRestored: false,
    fixtureServerRetainedForSaveCleanup: false,
  };
  ...
  try {
    ...
    await waitForCount(
      ...
      1,
      'core_update_fill_update_submit_not_observed_once',
    );
    ...
    await adapter.trustedClick(updateSelector, {
      outcome: (document) =>
    ...
    assert.equal(
      fixture.state.updateSubmissions,
      1,
      'core_update_fill_update_choice_submitted_fixture',
    );
    assert.equal(
      await verifyOwnedItemValues({
        itemId: ownedItemId,
        username,
      ...
      true,
      'core_update_fill_updated_values_not_materialized',
    );
    ...
    await wdPost(base, `/session/${sessionId}/window`, { handle: fillTab });
    assert.equal(
      await valuesMatch(driver, username, nextPassword),
      true,
      'core_update_fill_filled_values_mismatch',
    );
      ...
      if (tab)
        try {
          ...
          assert.equal(
            ...
            false,
            'core_update_fill_fixture_tab_still_open',
          );
          proof.coreUpdateFill[key] = true;
        } catch (error) {
          cleanup ||= error;
        }
    if (original)
      try {
        ...
        proof.coreUpdateFill.originalWindowRestored = true;
      } catch (error) {
        cleanup ||= error;
      }
  ...
  evidence.ok =
    evidence.updateSubmittedOnce &&
    evidence.exactUpdateChoiceVisible &&
    evidence.trustedUpdateClicked &&
    evidence.pendingRemoved &&
    evidence.updatedValuesVerified &&
    evidence.freshFillFocused &&
    evidence.exactFillVisible &&
    evidence.trustedFillClicked &&
    evidence.filledExactValues &&
    evidence.noExtraSubmission &&
    evidence.updateTabClosed &&
    evidence.fillTabClosed &&
    evidence.originalWindowRestored &&
    evidence.fixtureServerRetainedForSaveCleanup;
