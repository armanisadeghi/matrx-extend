...
function assertHelperDependencies({
  adapter,
  base,
  sessionId,
  wdPost,
  ...
  assert.ok(
    ...
    'generator_adapter_contract_invalid',
  );
  assert.ok(
    typeof base === 'string' && base.startsWith('http'),
    'generator_webdriver_base_invalid',
  );
  assert.ok(
    typeof sessionId === 'string' && sessionId.length > 0,
    'generator_webdriver_session_invalid',
  );
...
async function sidebarButton(adapter, label) {
  const selector = await adapter.waitFor(
    (document, rootSelector, exactLabel) => {
      const root = document.querySelector(rootSelector);
      if (!root) return null;
      const matches = [...root.querySelectorAll('button')].filter(
        (button) => button.textContent?.trim() === exactLabel,
      );
...
async function fixtureState({ base, sessionId, wdPost }, expectedGeneratedValue = null) {
  return wdPost(base, `/session/${sessionId}/execute/sync`, {
    script: `
...
async function runKind({ adapter, fixture, driver, kind }) {
  ...
  await adapter.trustedClick(generateButton, {
    outcome: (document, rootSelector) =>
      !!document
        ...
        ?.querySelector('[aria-label="Reveal generated value"]'),
    outcomeArgs: [GENERATOR],
  });
  await waitForGenerated(adapter);
  const masked = await adapter.evaluate(
    (document, rootSelector) => {
      const code = document.querySelector(rootSelector)?.querySelector('code')?.textContent;
      return (
        typeof code === 'string' &&
        code.length > 0 &&
        [...code].every((character) => character.charCodeAt(0) === 0x2022)
      );
    },
    [GENERATOR],
  );
  ...
  await adapter.trustedClick(`${GENERATOR} [aria-label="Reveal generated value"]`, {
    outcome: (document, rootSelector) =>
      !!document.querySelector(rootSelector)?.querySelector('[aria-label="Hide generated value"]'),
    outcomeArgs: [GENERATOR],
  });
  ...
  assert.equal(
    ...
    true,
    'generator_reveal_missing_value',
  );
  const firstMatchesDefaultShape =
    kind === 'Password'
      ? first.length === 24
  ...
  const regenerationDiffers =
    typeof regenerated === 'string' && regenerated.length > 0 && regenerated !== first;
  assert.equal(regenerationDiffers, true, 'generator_regenerate_reused_value');
  const regeneratedMatchesDefaultShape =
    kind === 'Password'
      ? regenerated.length === 24
      : regenerated.split('-').length === 6 &&
        regenerated.split('-').every((word) => word.length > 0);
  ...
  await adapter.trustedClick(useButton, {
    outcome: (document, rootSelector) => {
      ...
      return (
        !root?.querySelector('code') &&
        root?.querySelector('[aria-live="polite"]')?.textContent?.includes('Filled')
      );
    },
    outcomeArgs: [GENERATOR],
  });
  ...
  assert.equal(
    filled.newAndConfirmEqualExpected,
    true,
    'generator_new_password_fields_not_regenerated_value',
  );
...
export async function runFirefoxGeneratorChecks(dependencies) {
  ...
  try {
    ...
    assert.ok(
      ...
      'generator_original_window_missing',
    );
    tabHandle = (await wdPost(base, `/session/${sessionId}/window/new`, { type: 'tab' }))?.handle;
    assert.ok(
      typeof tabHandle === 'string' && tabHandle.length > 0,
      'generator_fixture_tab_missing',
    );
    ...
    await adapter.waitFor(
      (document) => !!document.querySelector('[aria-label="Password generator"]'),
    );
    const opener = await sidebarButton(adapter, 'Password generator');
    const open = await adapter.evaluate(
      (document, selector) =>
    ...
    if (
      ...
      !cleanupError &&
      workPassed &&
      proof.generator.tabClosed &&
      proof.generator.originalWindowRestored &&
      proof.generator.fixtureServerClosed
    )
