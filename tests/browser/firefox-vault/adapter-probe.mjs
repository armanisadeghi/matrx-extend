...
async function post(base, path, body) {
  ...
  if (!response.ok || data.value?.error)
    throw new Error(
      `webdriver_${String(data.value?.error ?? response.status)
        .replace(/[^a-z0-9]+/gi, '_')
        .toLowerCase()}`,
    );
  return data.value;
...
const [adapterSha256, harnessSha256, artifactXpiSha256, applicationIni, driverVersion] =
  await Promise.all([
...
try {
  ...
  assert.ok(
    typeof extensionBase === 'string' && extensionBase.startsWith('moz-extension://'),
    'native_sidebar_open_failed',
  );
  ...
  const keyboardInput = await adapter.evaluate((document) => {
    ...
    input.addEventListener('keydown', (event) => {
      ...
      input.dataset.keydownAllTrusted = String(
        input.dataset.keydownAllTrusted === 'true' && event.isTrusted,
      );
    });
    input.addEventListener('input', (event) => {
      input.dataset.inputCount = String(Number(input.dataset.inputCount || 0) + 1);
      input.dataset.inputAllTrusted = String(
        input.dataset.inputAllTrusted === 'true' && event.isTrusted,
      );
    });
  ...
  const beforeRoute = await adapter.evaluate(
    (document, selector) => {
  ...
  assert.deepEqual(
    beforeRoute,
    { targetFocused: true, valueLength: 0, inputCount: 0 },
    'keyboard_without_sidebar_focus_changed_target',
  );
  assert.equal(await adapter.focusOwnedSidebar(), true, 'keyboard_sidebar_focus_route_failed');
  const afterFocusRoute = await adapter.evaluate(
    (document, selector) => document.querySelector(selector) === document.activeElement,
    [keyboardInput],
  );
  assert.equal(afterFocusRoute, true, 'keyboard_sidebar_focus_changed_target');
  await performKeyboardActions(['p', 'r', 'o', 'b', 'e']);
  const afterRoute = await adapter.evaluate(
    (document, selector) => {
      const input = document.querySelector(selector);
      return {
  ...
  assert.equal(
    afterRoute?.targetFocused,
    true,
    'keyboard_sidebar_focus_changed_target_after_input',
  );
  ...
  const inputRemoved = await adapter.evaluate(
    (document, selector) => {
  ...
  const unicode = await adapter.evaluate(
    (document, value) => ({
  ...
  proof.unicodeTransport = {
    ...
    argumentCodePointsMatch:
      JSON.stringify(unicode?.argumentCodePoints) === JSON.stringify(unicodeOracle),
  ...
  assert.deepEqual(
    unicode?.argumentCodePoints,
    unicodeOracle,
    'remote_unicode_argument_codepoints_mismatch',
  );
  ...
  const authStorageEmpty = await post(base, `/session/${sessionId}/execute/async`, {
    script: `
    const done = arguments[arguments.length - 1];
    ...
    storage.local.get(keys).then(found => done(keys.every(key => !(key in found))), () => done(false));`,
    args: [],
  });
  ...
  await executeChromeAsync(
    ...
    win.SidebarController.show(sidebarId).then(done, () => done(false));`,
    [addonId],
  );
  ...
  await adapter.waitFor((document) => {
    ...
    return (
      !!settings &&
      !!chat &&
      settings.getAttribute('data-state') === 'inactive' &&
      chat.getAttribute('data-state') === 'active'
    );
  });
  ...
  assert.deepEqual(
    settingsClick.events.map((event) => [event.type, event.trusted]),
    [
      ['mousedown', true],
      ['mouseup', true],
      ['click', true],
  ...
  const chatClick = await adapter.trustedClick('button[title="Chat"]', {
    outcome: (document) =>
      document.querySelector('button[title="Settings"]')?.getAttribute('data-state') ===
        'inactive' &&
      document.querySelector('button[title="Chat"]')?.getAttribute('data-state') === 'active',
  });
  assert.deepEqual(
    chatClick.events.map((event) => [event.type, event.trusted]),
    [
      ['mousedown', true],
      ['mouseup', true],
      ['click', true],
  ...
  const overlayTarget = await adapter.evaluate((document) => {
    ...
    button.id = 'matrx-probe-covered-button';
    button.type = 'button';
    button.style.cssText =
      'position:fixed;left:20px;top:20px;width:160px;height:40px;z-index:2147483646';
    button.textContent = 'Disposable covered action';
    button.addEventListener('click', (event) => {
      if (event.isTrusted) button.dataset.clicked = 'true';
    });
    const cover = document.createElement('div');
    cover.id = 'matrx-probe-cover';
  ...
  const uncoveredClick = await adapter.trustedClick(overlayTarget, {
    outcome: (document) =>
      document.querySelector('#matrx-probe-covered-button')?.dataset.clicked === 'true',
    timeoutMs: 3000,
  });
  assert.ok(uncoveredClick.diagnostic.actionability.settleMs >= 300);
  assert.ok(
    uncoveredClick.events.every(
      (event) => event.trusted && ['exact', 'descendant'].includes(event.targetRelation),
    ),
  );
  ...
  await assert.rejects(
    () => adapter.trustedClick(overlayTarget, { timeoutMs: 750 }),
    /trusted_click_target_occluded/,
  );
  assert.equal(
    await adapter.evaluate(
      (document) => document.querySelector('#matrx-probe-covered-button')?.dataset.clicked,
    ),
    'false',
  );
  ...
  const portalControl = await adapter.evaluate((document) => {
    ...
    occluder.style.cssText =
      'position:fixed;left:0;top:0;width:220px;height:100px;z-index:2147483647';
  ...
  const portalPress = await adapter.trustedPress(portalControl, {
    outcome: (document) =>
      document.querySelector('[data-matrx-adapter-probe="portal"][role="listbox"]') !== null,
  });
  assert.deepEqual(
    portalPress.events.map((event) => [event.type, event.trusted]),
    [
      ['mousedown', true],
      ['mouseup', true],
      ['click', true],
  ...
  const appearance = await adapter.evaluate((document) => {
    const buttons = [...document.querySelectorAll('button[aria-controls]')].filter(
      (node) => node.textContent.trim() === 'Appearance',
    );
    if (buttons.length !== 1) return null;
    const id = buttons[0].getAttribute('aria-controls');
    const escaped = document.defaultView.CSS.escape(id);
    return {
      header: `button[aria-controls="${escaped}"]`,
      select: `#${escaped} [role="combobox"]`,
      expanded: buttons[0].getAttribute('aria-expanded'),
    };
  });
  assert.ok(appearance, 'appearance_section_missing');
  if (appearance.expanded === 'false')
    await adapter.trustedClick(appearance.header, {
      outcome: (document, selector) =>
        document.querySelector(selector)?.getAttribute('aria-expanded') === 'true',
      outcomeArgs: [appearance.header],
    });
  await adapter.trustedPress(appearance.select, {
    outcome: (document) => !!document.querySelector('[role="listbox"]'),
  });
  ...
  assert.equal(
    started.captureContract,
    'method_origin_path_status_owner_only_no_headers_query_body',
  );
  assert.equal(await post(base, `/session/${sessionId}/moz/context`, { context: 'content' }), null);
  await post(base, `/session/${sessionId}/url`, { url: `${local.origin}/` });
  const status = await post(base, `/session/${sessionId}/execute/async`, {
    script: `
    const done = arguments[arguments.length - 1];
    fetch('/known-owned-request?${forbiddenQueryCanary}=1', { method: 'POST' }).then(response => done(response.status), () => done(null));`,
    args: [],
  });
  assert.equal(status, 204, 'owned_localhost_request_failed');
  assert.equal(await post(base, `/session/${sessionId}/moz/context`, { context: 'chrome' }), null);
  const observed = await adapter.readNetworkObserver();
  const known = observed.events.filter(
    (event) => event.pathname === '/known-owned-request' && event.method === 'POST',
  );
  assert.ok(
    known.some((event) => event.phase === 'request'),
    'known_localhost_request_not_observed',
  );
  assert.ok(
    known.some((event) => event.phase === 'response' && event.status === 204),
    'known_localhost_response_not_observed',
  );
  assert.equal(
    JSON.stringify(observed).includes(forbiddenQueryCanary),
    false,
    'network_observer_captured_query',
  );
  ...
  proof.networkObserver = {
    ...
    knownRequestPhases: known.map((event) => ({
      phase: event.phase,
      method: event.method,
  ...
  if (observerStarted && adapter) {
    try {
      const result = await adapter.disposeNetworkObserver();
      proof.observerDisposed = result.disposed === true;
      observerStarted = false;
    } catch {
      proof.observerDisposed = false;
    }
  }
  ...
  try {
    await local.close();
    proof.localServerClosed = true;
  } catch {
    proof.localServerClosed = false;
  }
  ...
  const required = [
    ...
    'nativeSidebarControllerRoute',
    'unicodeTransportVerified',
    'trustedSidebarKeyboardInput',
    'transientOverlayWaitedWithoutClickThrough',
    'permanentOverlayRefusedWithoutClick',
    'trustedSettingsTransition',
    'trustedChatRestore',
    'trustedPortalComboboxPress',
    'trustedKeyboardComboboxSelection',
    'knownLocalhostRequestObserved',
    'metadataOnlyObserver',
    'observerDisposed',
    'addonUninstalled',
    'sessionDeleted',
    'driverExited',
