...
const CLEANUP_ROOT = join(__dirname, '..', '..', '..', '.matrx', 'task1-active', `canonical-cleanup-source-${CLEANUP_COMMIT}`);
...
test('capture-save refuses a noncanonical cleanup source before credential custody', () => {
  const result = spawnSync(process.execPath, [join(__dirname, 'read-only-auth-driver.mjs'), '--capture-save'], {
    env: captureSaveEnv({ MATRX_FIREFOX_LOCAL_SOURCE_ROOT: '/tmp/not-canonical-cleanup-source' }), encoding: 'utf8', timeout: 10_000,
  });
  assert.notEqual(result.status, 0);
...
test('capture-save refuses a cleanup source hash mismatch before credential custody', () => {
  const result = spawnSync(process.execPath, [join(__dirname, 'read-only-auth-driver.mjs'), '--capture-save'], {
    env: captureSaveEnv({ MATRX_FIREFOX_LOCAL_ROUTER_SHA256: '0'.repeat(64) }), encoding: 'utf8', timeout: 10_000,
  });
  assert.notEqual(result.status, 0);
...
test('capture-save cleanup kills a hung child before reporting its fixed timeout', () => {
  const result = spawnSync(process.execPath, [join(__dirname, 'read-only-auth-driver.mjs'), '--capture-save-cleanup-timeout-self-test'], {
    env: { PATH: process.env.PATH }, encoding: 'utf8', timeout: 10_000,
  });
