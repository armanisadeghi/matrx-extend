...
const {
  createAuthenticatorPreservation,
  exerciseCanonicalPrimitive,
} = require('./vault-authenticator-preservation.cjs');

const itemId = 'receipt-owned-item';
const metadata = {
  credential_item_id: itemId,
  display_name: 'Receipt owned login',
  label: 'Receipt owned',
  issuer: 'Matrx Canary',
  digits: 8,
  period: 30,
  algorithm: 'SHA256',
  enabled: true,
  login_urls: ['https://example.invalid/login'],
  seed_field_id: 'sealed-seed-field',
};
...
async function successLifecycle() {
  ...
  const helper = createAuthenticatorPreservation({
    ...
    createEnrollmentInput: () => ({
      seed: 'PROCESS_ONLY_TEST_SEED',
      enrollmentInput: 'otpauth://test',
    }),
...
async function recentAuthRefusalDoesNotReachCode() {
  ...
  const helper = createAuthenticatorPreservation({
    ...
    verifyCode: async () => {
      throw new Error('recent auth refusal reached code verification');
    },
  });
...
async function ambiguousEnrollmentNeverRetries() {
  ...
  const helper = createAuthenticatorPreservation({
    request: async (request) => {
      calls.push(request.method + request.path);
      if (request.method === 'GET' && !request.path.endsWith('/code'))
        return calls.filter((call) => call === `GET/${itemId}`).length === 1
          ? { status: 404 }
  ...
  assert.equal(handle.enrolled, true);
  assert.equal(
    calls.filter((call) => call === 'POST/enroll').length,
    1,
    'ambiguous enrollment retried POST',
  );
...
async function validationFailureStillHasCleanupHandle() {
  ...
  await assert.rejects(
    () => helper.enroll({ credentialItemId: itemId }),
...
async function canonicalPrimitiveRunsWithoutReturningACode() {
  assert.equal(
    await exerciseCanonicalPrimitive({
...
async function cleanupReconcilesAfterJournalFailure() {
  ...
  const helper = createAuthenticatorPreservation({
    ...
    verifyCode: async () => true,
    journal: ({ method }) => {
      if (method === 'DELETE') throw new Error('journal');
    },
  });
  const handle = await helper.enroll({ credentialItemId: itemId });
  await assert.rejects(() => helper.cleanup(handle), /authenticator_journal_failed/);
  assert.equal(handle.cleanupProven, true);
  assert.equal(handle.acceptanceFailed, true);
  assert.equal(calls.at(-1), `GET/${itemId}`);
}
...
async function defaultVerifierRfcVectorAndRejections() {
  ...
  const lifecycle = (code) => {
    ...
    helper = createAuthenticatorPreservation({
      request: async (request) => {
        if (request.method === 'POST') return { status: 200, body: metadata };
        if (request.method === 'DELETE') return { status: 204 };
        if (request.path.endsWith('/code')) return { status: 200, body: { code } };
        gets += 1;
        if (gets === 1) return { status: 404 };
        return { status: 200, body: changed ? { ...metadata, issuer: 'changed' } : metadata };
      },
      markCleanupObligation: async () => {},
      createEnrollmentInput: enrollment,
      now: () => 59,
    });
    return {
      helper,
  ...
  const good = lifecycle('46119246');
  const accepted = await good.helper.enroll({ credentialItemId: itemId });
  ...
  const bad = lifecycle('00000000');
  await assert.rejects(
    () => bad.helper.enroll({ credentialItemId: itemId }),
    /authenticator_code_invalid/,
  );
  good.change();
  await assert.rejects(() => good.helper.afterUpdate(accepted), /authenticator_metadata_changed/);
}
...
(async () => {
  ...
  process.stdout.write(
    'PASS: enrolled authenticator lifecycle preserves exact metadata, validates codes, and never retries ambiguous enrollment\n',
  );
})().catch((error) => {
