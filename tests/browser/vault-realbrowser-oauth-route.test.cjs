...
const match = runner.match(/async function visibleExactCount[\s\S]*?\n}\n\nlet cdpWorkerMessageId/);
...
new vm.Script(`${functionSource}; globalThis.observe = awaitOAuthRouteOrCallback; globalThis.observeCallback = awaitOAuthCallbackStorage; globalThis.observeApproval = observeOAuthConsentApproval;`).runInContext(sandbox);
...
const page = ({ url = 'https://www.aimatrx.com/auth', email = false, password = false, authorize = [], retry = [], redirecting = [], headings = [], closed = false } = {}) => {
