  ...
  (globalThis as unknown as { chrome: unknown }).chrome = {
    webNavigation: {
      getFrame: async ({ frameId }: chrome.webNavigation.GetFrameDetails) =>
        frames.get(frameId) ?? null,
    },
    permissions: {
      contains: async ({ origins }: chrome.permissions.Permissions) =>
        origins?.every((origin) => allowed.has(origin)) === true,
    },
  };
});
...
describe('credential frame binding', () => {
  it('freezes permitted cross-origin ancestry and rejects changed ancestor identity or parent chain', async () => {
    const { credentialFrameBindingCurrent, readCredentialFrameBinding } = await import(
      '@/lib/credentials/frame-binding'
    );
    ...
    expect(binding?.chain.map((link) => [link.frameId, link.documentId, link.origin])).toEqual([
      [4, 'child-a', 'https://login.example.test'],
      [0, 'top-a', 'https://app.example.test'],
    ]);
  ...
  it.each([
    ...
    [
      'opaque child URL',
      () => frames.set(4, { documentId: 'child-a', url: 'about:blank', parentFrameId: 0 }),
    ],
