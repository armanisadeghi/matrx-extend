...
const ready = () => ({
  status: 'ready', offerId: 'a'.repeat(36), itemIds: [first, second],
  matches: [{ item_id: first, display_name: 'First account' }, { item_id: second, display_name: 'Second account' }],
  pageUrl: 'https://child.example/login', frameId: 3,
});
...
describe('saved-login panel metadata boundary', () => {
  ...
  it.each([
    ...
    ['duplicate matched accounts', { ...ready(), matches: [ready().matches[0], ready().matches[0]] }],
    ['mismatched account', { ...ready(), matches: [{ item_id: first, display_name: 'First account' }] }],
    ['unexpected secret field', { ...ready(), matches: [{ ...ready().matches[0], password: 'forbidden' }, ready().matches[1]] }],
    ...
    ['extra target on none', { status: 'none', itemIds: [], pageUrl: 'https://child.example/login' }],
