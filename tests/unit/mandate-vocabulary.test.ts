...
describe('mandate vocabulary', () => {
  ...
  it('sources them from the package, not from a local literal', () => {
    ...
    expect(STRUCTURED_EXTRACTOR_MANDATE_KEY).toBe(
      MANDATE_KEYS.extend__structured_extractor,
    );
    expect(PATTERN_FROM_DATA_MANDATE_KEY).toBe(
  ...
  it('refs carry the key and resolve back to it', () => {
    ...
    expect(STRUCTURED_EXTRACTOR_MANDATE_REF).toBe(
      `mandate:${STRUCTURED_EXTRACTOR_MANDATE_KEY}`,
    );
