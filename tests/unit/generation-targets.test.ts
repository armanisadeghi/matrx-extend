...
describe('generated password DOM primitive', () => {
  ...
  it('refuses generated-password offers for unsafe form destinations and submitter overrides', () => {
    ...
    mount(
      '<form id=visual method=post action="/change"><fieldset><input id=new type=password autocomplete=new-password form=external></fieldset></form><form id=external method=post action="https://elsewhere.example/change"></form>',
    );
    ...
    Object.defineProperty(externallyAssociated, 'form', {
      configurable: true,
      get: () => associated,
    });
    mountGenerationTargetRegistry();
  ...
  it('refuses an action changed after offer and rolls back when an input event changes the form association', () => {
    const form = mount(
      '<form id=safe method=post action="/change"><fieldset><input id=new type=password autocomplete=new-password><input id=confirm type=password autocomplete=new-password aria-label="confirm password"></fieldset></form><form id=unsafe method=post action="https://elsewhere.example/change"></form>',
    );
    mountGenerationTargetRegistry();
    const expiry = expiresAt();
    const targets = discover(expiry).groups[0]?.targets ?? [];
  ...
  it('prevalidates native constraints and does not truncate a generated value', () => {
    mount(
      '<form method=post action="/change"><input id=new type=password autocomplete=new-password maxlength=8></form>',
    );
    mountGenerationTargetRegistry();
    const expiry = expiresAt();
  ...
  it('treats an empty native pattern as a constraint instead of silently ignoring it', () => {
    mount(
      '<form method=post action="/change"><input id=new type=password autocomplete=new-password pattern=""></form>',
    );
    mountGenerationTargetRegistry();
    const expiry = expiresAt();
    const target = discover(expiry).groups[0]?.targets[0];
  ...
  it('rolls back only its own write when an event changes constraints before the next target', () => {
    mount(
      '<form method=post action="/change"><input id=new type=password autocomplete=new-password><input id=confirm type=password autocomplete=new-password aria-label="confirm password"></form>',
    );
    mountGenerationTargetRegistry();
    const expiry = expiresAt();
    const targets = discover(expiry).groups[0]?.targets ?? [];
  ...
  it('refuses a moved original node and a changed confirmation role after synchronous events', () => {
    const form = mount(
      '<form method=post action="/change"><fieldset id=group><input id=new type=password autocomplete=new-password><input id=confirm type=password autocomplete=new-password aria-label="confirm password"></fieldset></form>',
    );
    mountGenerationTargetRegistry();
    const expiry = expiresAt();
    const targets = discover(expiry).groups[0]?.targets ?? [];
  ...
  it('claims target identities before the first write so a replay cannot fill', () => {
    mount(
      '<form method=post action="/change"><input id=new type=password autocomplete=new-password></form>',
    );
    mountGenerationTargetRegistry();
    const expiry = expiresAt();
    const targets = discover(expiry).groups[0]?.targets ?? [];
  ...
  it('refuses generated fill when the isolated registry cannot mark the target sensitive', () => {
    ...
    expect(result).toEqual({ status: 'refused_unchanged', reason: 'registry_unavailable' });
  ...
  it('refuses a caller-supplied subset of an offered confirmation group', () => {
    mount(
      '<form method=post action="/change"><input type=password autocomplete=new-password><input type=password autocomplete=new-password aria-label="confirm password"></form>',
    );
    mountGenerationTargetRegistry();
    const expiry = expiresAt();
    const targets = discover(expiry).groups[0]?.targets ?? [];
  ...
  it('offers explicit password groups separately and refuses an added member after discovery', () => {
    mount(
      '<form method=post action="/change"><fieldset><input type=password autocomplete=new-password></fieldset><fieldset><input type=password autocomplete=new-password></fieldset></form>',
    );
    mountGenerationTargetRegistry();
    const expiry = expiresAt();
  ...
  it('does not read a password value while discovering metadata', () => {
    ...
    Object.defineProperty(input, 'value', {
      configurable: true,
      get: () => {
        throw new Error('forbidden_value_read');
      },
      set: () => undefined,
    });
    mountGenerationTargetRegistry();
  ...
  it('refuses an expiry beyond the generator TTL ceiling', () => {
    ...
    mountGenerationTargetRegistry();
    expect(discover(Date.now() + GENERATED_SECRET_TTL_MS * 2)).toMatchObject({
      groups: [],
  ...
  it('rolls back an operation value when the native controlled setter writes then throws', () => {
    mount(
      '<form method=post action="/change"><input id=new type=password autocomplete=new-password value="original"></form>',
    );
    mountGenerationTargetRegistry();
    const expiry = expiresAt();
    const targets = discover(expiry).groups[0]?.targets ?? [];
    ...
    Object.defineProperty(layer, 'value', {
      set(next: string) {
        native.call(this, next);
        if (throws) {
          throws = false;
          throw new Error('setter_after_write');
        }
      },
      get() {
