...
describe('credential form destination classifier', () => {
  ...
  it.each([long, short])('accepts exact deployed React literal', (action) => {
    expect(
      classify(
        mount(`<form action="${action.replaceAll('"', '&quot;')}"><input type=password></form>`),
      ).kind,
    ).toBe('react_action');
  });
  ...
  it('refuses default GET and enabled unsafe override census', () => {
    ...
    expect(
      classify(
        mount(
          '<form method=post><input type=password><button type=submit formmethod=get>go</button></form>',
        ),
      ).kind,
    ).toBe('unsafe');
  });
...
it('revalidates legacy fill after focus mutates its destination', () => {
  ...
  const outcome = serialized({
    operation: 'fill',
    expected: null,
    requested: [{ selector: '#user', value: 'secret' }],
    sensitiveAttr: '',
...
it('refuses an OTP/new-password bound group before writing', () => {
  document.body.innerHTML =
    '<form method="post"><input id="otp" autocomplete="one-time-code"><input id="password" type="password"></form>';
