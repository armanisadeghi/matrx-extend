...
beforeEach(() => {
  ...
  Object.assign(chrome, {
    ...
    action: { setBadgeText: async (value: { tabId: number; text: string }) => { badgeWrites.push(value); await badgeGate; }, setTitle: async (value: { tabId: number; title: string }) => titleWrites.push(value) },
    tabs: { query: async () => { await tabsGate; return tabs; }, onRemoved: { addListener: () => undefined }, onUpdated: { addListener: () => undefined } },
  });
});
...
describe('credential assistance status', () => {
  ...
  it('does not let a boot clear overwrite a newer saved-login projection while Chrome writes are deferred', async () => {
    ...
    badgeGate = new Promise((resolve) => { release = resolve; });
    tabsGate = new Promise((resolve) => { releaseTabs = resolve; });
