...
function fixture(mode) {
  ...
  for (const [dir, version] of [
    [source, '0.3.2'],
    [destinations[0], '0.3.0'],
    [destinations[1], '0.3.1'],
  ]) {
    mkdirSync(dir);
    writeFileSync(
      join(dir, 'manifest.json'),
      JSON.stringify({ key: 'local-development-key', version }),
    );
...
function run(value) {
  return spawnSync(
    ...
    [
      '--import',
      value.preload,
      resolve('scripts/sync-unpacked-release.mjs'),
      '--root',
      value.root,
      '--source',
      value.source,
      ...
      '--store-zip',
      join(value.root, 'store.zip'),
      '--local-zip',
      join(value.root, 'local.zip'),
      '--receipt',
      value.receipt,
    ],
    { encoding: 'utf8' },
...
for (const mode of ['cleanup', 'receipt-cleanup']) {
  test(`${mode} failure keeps committed bundles and receipt coherent and exits successfully`, () => {
    ...
    try {
      ...
      for (const dir of value.destinations)
        assert.equal(readFileSync(join(dir, 'sidepanel.js'), 'utf8'), 'extension release 0.3.2');
      if (mode === 'cleanup') {
        const backup = readdirSync(value.root).find((name) =>
          name.startsWith('.chrome-mv3.release-backup-'),
        );
...
test('failed rollback retains the unrecovered backup while restoring the independent destination', () => {
  ...
  try {
    ...
    assert.equal(manifestVersion(join(value.root, backup)), '0.3.1');
    assert.ok(
      result.stderr.includes(join(value.root, backup)),
      'error names the recoverable prior bytes',
    );
    assert.equal(readFileSync(value.receipt, 'utf8'), '{"version":"0.3.1"}\n');
  } finally {
    rmSync(value.root, { recursive: true, force: true });
