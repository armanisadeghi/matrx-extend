 ...
 * This extension already signs out with `/auth/v1/logout?scope=local` (flow.ts); this
...
const SKIP = /(^|\/)(node_modules|dist|target|build|\.next|\.venv|__pycache__|\.matrx|release)\//;
...
export function findingsIn(source, file) {
  ...
  lines.forEach((line, i) => {
    ...
    for (const m of /\.(rs|py)$/.test(file) ? [] : line.matchAll(SIGN_OUT)) {
      ...
      if (!scope)
        out.push(`${where}: .signOut( without a literal scope (supabase-js defaults to GLOBAL)`);
      else if (scope.groups.scope === 'global')
        out.push(`${where}: explicit scope "global" sign-out`);
    }
    for (const m of line.matchAll(LOGOUT_URL)) {
      const rest = m.groups.rest ?? '';
      if (!/scope=/.test(rest) && !/\{scope\}/.test(rest))
        out.push(`${where}: GoTrue logout URL without scope= (defaults to GLOBAL)`);
  ...
  return execSync('git ls-files --cached --others --exclude-standard', {
    ...
    .filter(
      (f) =>
        ...
        !/\.test\.|self-test/.test(f) &&
        !f.endsWith('check-signout-scope.mjs'),
    );
}
...
if (isMain) {
  ...
  console.log(
    'check:signout-scope: every sign-out in this repo names a literal local/others scope.',
  );
}
