...
import { readFile, writeFile, mkdir, rm, rmdir, rename, stat, statfs, readdir } from 'node:fs/promises';
...
const policy = JSON.parse(await readFile(join(repo, 'docs/stabilization/resource-policy.json'), 'utf8'));
...
for (let i = 0; i < raw.length; i++) {
  if (raw[i] === '--') { command = raw.slice(i + 1); break; }
  if (!raw[i].startsWith('--') || i + 1 >= raw.length) throw new Error('RESOURCE_ARGUMENT_INVALID');
  flags.set(raw[i], raw[++i]);
}
const emit = (code, extra = {}) => console.log(JSON.stringify({ schema: 1, at: new Date().toISOString(), ...extra, code }));
const fail = (code, extra = {}) => { emit(code, extra); process.exitCode = 2; };
const number = (value, name) => {
  const n = Number(value);
  if (!Number.isFinite(n) || n < 0) throw new Error(`RESOURCE_MEASUREMENT_INVALID:${name}`);
  return n;
};
const output = async (cmd, args) => (await run(cmd, args, { timeout: 5000, maxBuffer: policy.processSnapshotMaxBytes })).stdout.trim();
const sysctl = (name) => output('/usr/sbin/sysctl', ['-n', name]);
const sleep = (ms) => new Promise((done) => setTimeout(done, ms));
const GiB = 1024 ** 3;
const positive = (key) => Number.isFinite(policy[key]) && policy[key] > 0;
function validatePolicy() {
  if (policy.schema !== 1 || !Array.isArray(policy.allowedCommands) || !policy.allowedCommands.length || !policy.allowedCommands.every((command) => Array.isArray(command) && command.length && command.every((part) => typeof part === 'string' && part.length)) || ![
    'processSnapshotMaxBytes', 'watchIntervalSeconds', 'swapWindowSeconds', 'unsafeSamplesToStop', 'healthySamplesToResume',
    'minimumAvailableMemoryGiB', 'minimumAvailableMemoryFraction', 'maximumOneMinuteLoadPerLogicalCpu',
    'maximumSustainedCpuBusyFraction', 'maximumBusyFractionAtHighLoad', 'cpuSampleIntervalSeconds',
    'ownedGroupGraceSeconds', 'ownedGroupKillWaitSeconds', 'minimumFreeDiskGiB', 'maximumSwapGrowthMiB',
    'normalMacPressureLevel',
  ].every(positive) || policy.cpuSampleIntervalSeconds > policy.swapWindowSeconds ||
    policy.maximumSustainedCpuBusyFraction > 1 || policy.maximumBusyFractionAtHighLoad > 1 ||
...
async function groupMembers(groupId) {
  ...
  return raw.split('\n').map((line) => line.trim().split(/\s+/)).filter((parts) =>
    parts.length >= 3 && Number(parts[1]) === groupId && !parts[2].startsWith('Z'),
...
async function stopOwnedProcesses(owner) {
  ...
  for (const [signal, seconds] of [['SIGTERM', policy.ownedGroupGraceSeconds], ['SIGKILL', policy.ownedGroupKillWaitSeconds]]) {
    const deadline = Date.now() + seconds * 1000;
    do {
...
function validateCommand() {
  if (mode !== 'run') return;
  const exact = policy.allowedCommands.some((allowed) => JSON.stringify(command) === JSON.stringify(allowed));
  ...
  const filteredTest = testPrefix.every((part, i) => command[i] === part) &&
    command.slice(testPrefix.length).every((part) => /^[A-Za-z0-9_./-]+\.(?:test|spec)\.[cm]?[jt]sx?$/.test(part) && !part.startsWith('-'));
  if (!exact && !filteredTest) throw new Error('RESOURCE_COMMAND_NOT_ALLOWED:see docs/stabilization/resource-policy.json');
}

async function sample(profileDir) {
  if (platform() !== 'darwin') throw new Error('RESOURCE_UNSUPPORTED_PLATFORM');
  const [memory, pressure, total, cpus, load, swap, repoDisk, profileDisk, cpuBusy] = await Promise.all([
  ...
  const freePercent = number(memory.match(/System-wide memory free percentage:\s*([\d.]+)%/)?.[1], 'available-memory');
  const oneMinuteLoad = number(load.match(/\{\s*([\d.]+)/)?.[1], 'load');
  const swapUsedMiB = number(swap.match(/used\s*=\s*([\d.]+)([MG])/)?.[1], 'swap') * (swap.match(/used\s*=\s*([\d.]+)([MG])/)?.[2] === 'G' ? 1024 : 1);
  const logicalCpus = number(cpus, 'logical-cpus');
  if (logicalCpus === 0 || freePercent > 100) throw new Error('RESOURCE_MEASUREMENT_INVALID:cpu-or-memory');
  return {
    pressureLevel: number(pressure, 'pressure'),
    availableMemoryGiB: number(total, 'physical-memory') * freePercent / 100 / GiB,
    requiredMemoryGiB: Math.max(policy.minimumAvailableMemoryGiB, number(total, 'physical-memory') * policy.minimumAvailableMemoryFraction / GiB),
    oneMinuteLoad,
    logicalCpus,
    cpuBusyFraction: cpuBusy,
    swapUsedMiB,
    repoFreeGiB: number(Number(repoDisk.bavail) * Number(repoDisk.bsize), 'repo-disk') / GiB,
    profileFreeGiB: number(Number(profileDisk.bavail) * Number(profileDisk.bsize), 'profile-disk') / GiB,
  };
}
...
function reasons(current, previous) {
  ...
  if (current.cpuBusyFraction >= policy.maximumSustainedCpuBusyFraction) bad.push('RESOURCE_CPU_BUSY');
  if (current.oneMinuteLoad / current.logicalCpus >= policy.maximumOneMinuteLoadPerLogicalCpu &&
      current.cpuBusyFraction >= policy.maximumBusyFractionAtHighLoad) bad.push('RESOURCE_CPU_HIGH_LOAD_BUSY');
  if (current.repoFreeGiB < policy.minimumFreeDiskGiB || current.profileFreeGiB < policy.minimumFreeDiskGiB) bad.push('RESOURCE_DISK_LOW');
  if (!previous) bad.push('RESOURCE_SWAP_BASELINE_MISSING');
  else if (current.swapUsedMiB - previous.swapUsedMiB > policy.maximumSwapGrowthMiB) bad.push('RESOURCE_SWAP_GROWTH');
  return bad;
}
...
async function preflight(profileDir) {
  ...
  const second = { ...samples.at(-1), oneMinuteLoad: Math.max(...samples.map((item) => item.oneMinuteLoad / item.logicalCpus)) * samples.at(-1).logicalCpus, cpuBusyFraction: samples.reduce((sum, item) => sum + item.cpuBusyFraction, 0) / samples.length };
  const allReasons = [...new Set(samples.flatMap((item) => reasons(item, first).filter((reason) => !reason.startsWith('RESOURCE_CPU_'))))];
  allReasons.push(...reasons(second, first).filter((reason) => reason.startsWith('RESOURCE_CPU_')));
  return { sample: second, cpuBusySamples: samples.map((item) => item.cpuBusyFraction), reasons: allReasons, swapWindowSeconds: policy.swapWindowSeconds };
}
...
async function recoverHold(profileDir) {
  ...
  while (hold.healthySamples < policy.healthySamplesToResume) {
    ...
    if (bad.length) {
      await writeFile(holdPath, JSON.stringify({ ...hold, healthySamples: 0, reason: bad }) + '\n', { mode: 0o600 });
...
async function acquire(owner) {
  try { await mkdir(lock, { mode: 0o700 }); }
    ...
    try { await mkdir(reclaimLock, { mode: 0o700 }); }
    catch (lockError) {
      if (lockError.code === 'EEXIST') throw new Error('RESOURCE_RECLAIM_BUSY');
      throw lockError;
    }
    try {
      const prior = await readOwner().catch(() => { throw new Error('RESOURCE_OWNER_UNREADABLE'); });
      ...
      if (prior.kind === 'browser') throw new Error('RESOURCE_STALE_BROWSER:stop manual UI work then use recover-browser --confirm-no-owned-work yes');
      if (prior.childPending || prior.groupId) {
        if (prior.ownership !== 'inherited-token-v1') throw new Error('RESOURCE_STALE_OWNER_NEEDS_REVIEW');
        if (!await stopOwnedProcesses(prior)) throw new Error('RESOURCE_OWNED_PROCESS_STILL_RUNNING');
      }
      if (prior.groupId && !await groupGone(prior.groupId)) throw new Error('RESOURCE_GROUP_STILL_RUNNING');
      const retired = join(root, `retired-${prior.nonce}`);
      await rename(lock, retired);
      await mkdir(lock, { mode: 0o700 });
      await rm(retired, { recursive: true });
    } finally { await rm(reclaimLock, { recursive: true }); }
  }
  try { await writeFile(join(lock, 'owner.json'), JSON.stringify(owner) + '\n', { flag: 'wx', mode: 0o600 }); }
  catch (error) { throw new Error(`RESOURCE_OWNER_WRITE_FAILED:${error.code ?? error.message}`); }
}

async function release(owner) {
  const current = await readOwner();
  if (current.nonce !== owner.nonce || current.processStart !== owner.processStart) throw new Error('RESOURCE_OWNER_CHANGED');
  await rm(join(lock, 'owner.json'));
  await rmdir(lock);
}

async function recoverBrowser() {
  if (flags.get('--confirm-no-owned-work') !== 'yes') throw new Error('RESOURCE_RECOVERY_CONFIRMATION_REQUIRED');
  await mkdir(root, { recursive: true, mode: 0o700 });
  await mkdir(reclaimLock, { mode: 0o700 });
  try {
    const prior = await readOwner();
    if (prior.kind !== 'browser' || !prior.processStart || !prior.nonce || await processIdentity(prior.pid) === prior.processStart)
      throw new Error('RESOURCE_BROWSER_RECOVERY_NOT_PROVEN');
    await release(prior);
    emit('RESOURCE_BROWSER_LEASE_RETIRED');
  } finally { await rmdir(reclaimLock); }
}
...
async function recoverOwnerless() {
  ...
  await mkdir(root, { recursive: true, mode: 0o700 });
  try { await mkdir(reclaimLock, { mode: 0o700 }); }
  catch (error) { throw new Error(error.code === 'EEXIST' ? 'RESOURCE_RECLAIM_BUSY' : 'RESOURCE_RECOVERY_FAILED'); }
  try {
    const lockStat = await stat(lock).catch(() => { throw new Error('RESOURCE_OWNERLESS_NOT_PROVEN'); });
    if (!lockStat.isDirectory() || Date.now() - lockStat.mtimeMs < 30_000 || (await readdir(lock)).length)
    ...
    const anotherRunner = processes.split('\n').some((line) => {
      ...
      return match && Number(match[1]) !== process.pid && /(?:^|\/)node\s/.test(match[2]) && match[2].includes('stabilization-resource.mjs');
    });
...
async function main() {
  if (!['check', 'run', 'browser', 'recover-ownerless', 'recover-browser'].includes(mode) || (mode === 'run' && !command.length) || (mode !== 'run' && command.length)) throw new Error('RESOURCE_ARGUMENT_INVALID');
  validatePolicy();
  validateCommand();
  ...
  if (mode !== 'check' && (!runId || !/^[A-Za-z0-9_.-]+$/.test(runId))) throw new Error('RESOURCE_RUN_ID_INVALID');
  ...
  const owner = { schema: 1, pid: process.pid, processStart: await processIdentity(process.pid), runId: runId ?? 'check', kind: mode, nonce: randomUUID(), acquiredAt: new Date().toISOString() };
  ...
  let stop = false;
  let wakeStop;
  const stopEvent = new Promise((done) => { wakeStop = done; });
  let unsafe = 0;
  let healthy = 0;
  let previous;
  let swapWindowAt = 0;
  ...
  try {
    ...
    if (pre.reasons.length) { fail('RESOURCE_PREFLIGHT_REFUSED', { runId, ...pre }); return; }
    if (stop) { fail('RESOURCE_STOP_REQUESTED', { runId }); return; }
    previous = pre.sample;
    ...
    if (mode === 'run') {
      ...
      await writeFile(join(lock, 'owner.json'), JSON.stringify(owner) + '\n', { mode: 0o600 });
      child = spawn(command[0], command.slice(1), { detached: true, stdio: 'inherit', cwd: repo, env: { ...process.env, MATRX_RESOURCE_OWNER: owner.nonce, MATRX_RESOURCE_RUN_ID: runId, MATRX_RESOURCE_STOP_FILE: join(root, `stop-${owner.nonce}.json`) } });
      // Attach lifecycle listeners before the first await: /usr/bin/true may exit immediately.
      settled = new Promise((done) => {
        child.once('error', (error) => done({ childError: error.message, childExitCode: null, childSignal: null }));
    ...
    while (true) {
      const result = await Promise.race([sleep(policy.watchIntervalSeconds * 1000).then(() => null), settled ?? new Promise(() => {}), stopEvent]);
      if (result?.stop) {
        if (mode === 'browser') {
        ...
        if (groupId && !await stopOwnedGroup(groupId, runId, 'operator-signal')) throw new Error('RESOURCE_GROUP_UNCONFIRMED');
        process.exitCode = resourceInvalid ? 3 : 130;
        break;
      }
      if (result) {
        emit('RESOURCE_JOB_EXIT', { runId, ...result });
        if (groupId && !await stopOwnedGroup(groupId, runId, 'child-exited-with-owned-descendants')) throw new Error('RESOURCE_GROUP_UNCONFIRMED');
        process.exitCode = resourceInvalid ? 3 : (result.childExitCode ?? 1);
        break;
      }
      let current; let bad;
      try { current = await sample(profileDir); bad = reasons(current, previous); }
      catch (error) { bad = [`RESOURCE_MEASUREMENT_FAILED:${error.message}`]; }
      if (current && Date.now() - swapWindowAt >= policy.swapWindowSeconds * 1000) {
        previous = current;
        swapWindowAt = Date.now();
      }
      if (bad.length) { unsafe++; healthy = 0; } else { unsafe = 0; healthy++; }
      ...
      emit(bad.length ? 'RESOURCE_WATCH_UNSAFE' : 'RESOURCE_WATCH_HEALTHY', { runId, reasons: bad, sample: current });
      if (unsafe >= policy.unsafeSamplesToStop) {
        const hold = { schema: 1, runId, reason: bad, at: new Date().toISOString(), healthySamples: 0 };
        await writeFile(holdPath, JSON.stringify(hold) + '\n', { mode: 0o600 });
        ...
        unsafe = 0;
        if (mode === 'run') {
          if (groupId && !await stopOwnedGroup(groupId, runId, 'watchdog')) throw new Error('RESOURCE_GROUP_UNCONFIRMED');
          break;
        }
        emit('RESOURCE_BROWSER_OPERATOR_STOP_REQUIRED', { runId, instruction: 'Stop manual browser work, then press Ctrl-C on this lease holder.' });
      }
    ...
    if (owner.ownership && !await stopOwnedProcesses(owner)) {
      emit('RESOURCE_OWNED_PROCESS_STILL_RUNNING', { runId, recovery: 'Rerun the same guarded command; stale ownership cleanup is automatic.' });
      process.exitCode = 3;
...
try { await main(); }
catch (error) { fail(error.message.startsWith('RESOURCE_') ? error.message : 'RESOURCE_MEASUREMENT_FAILED', { detail: error.message }); }
