...
export default defineConfig({
  ...
  manifest: ({ mode, browser }) => {
    ...
    return {
      ...
      // Credential writes require documentIds (106+), and the user-facing
      // panel-opening flow requires sidePanel.open (116+). Advertise the
      // highest required floor so installation cannot promise a broken flow.
      ...
      ...(!isFirefox && !isChromeWebStoreBuild ? { key: devExtensionKey } : {}),
      permissions: [
        'storage',
        'sidePanel',
        'activeTab',
      ...
      optional_permissions: ['cookies', 'pageCapture', 'clipboardRead', 'tabCapture'].filter(
        (permission) => (isFirefox ? !['pageCapture', 'tabCapture'].includes(permission) : true),
      ),
